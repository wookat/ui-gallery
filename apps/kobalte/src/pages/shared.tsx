...
export function PageHeader(props: ParentProps<{ title: string; description?: string; action?: JSX.Element }>) {
  return <div class="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div class="min-w-0 space-y-1"><p class="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">ACME CONSOLE</p><h1 class="text-3xl font-semibold tracking-tight">{props.title}</h1>{props.description ? <p class="text-sm text-zinc-500 dark:text-zinc-400">{props.description}</p> : null}</div>{props.action}</div>
}
