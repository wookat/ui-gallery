        ...
        <Space direction="vertical" size={24} style={{ width: "100%" }}>
          ...
          <Form layout="vertical" onSubmit={submit}>
            <Form.Item label="邮箱" field="email" rules={[{ required: true, message: "请输入邮箱" }, { type: "email", message: "邮箱格式不正确" }]}>
              <Input size="large" prefix={<Icon name="user" />} placeholder="you@example.com" />
            </Form.Item>
            <Form.Item label="密码" field="password" rules={[{ required: true, message: "请输入密码" }]}>
              <Input.Password size="large" prefix={<Icon name="lock" />} placeholder="••••••••" visibilityToggle={!visible} onVisibilityChange={setVisible} />
            </Form.Item>
            <div className="between"><Checkbox>记住我</Checkbox><Link to="#forgot">忘记密码？</Link></div>
            <Button size="large" htmlType="submit" type="primary" long loading={loading} style={{ marginTop: 16 }}>登录</Button>
          </Form>
          <Divider>或</Divider>
          <Space wrap style={{ width: "100%" }}>
            <Button size="large" type="secondary" icon={<Icon name="globe" />}>Google</Button>
            <Button size="large" type="secondary" icon={<Icon name="github" />}>GitHub</Button>
            <Button size="large" type="secondary" icon={<Icon name="message-circle" />}>微信</Button>
          </Space>
