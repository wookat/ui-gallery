...
import { PageHeader, ScrollHint } from "./shared"
...
function Demo({ name, open, setOpen }: { name: string; open: boolean; setOpen: (value: boolean) => void }) {
  ...
  if (name === "Statistic") return <Stat><Stat.Label>收入</Stat.Label><Stat.Value value={128430} /><Stat.HelpText>较上月增长</Stat.HelpText><Text color="green" className="text-up">+12.4%</Text></Stat>
  ...
  if (name === "Carousel") return <div className="carousel-box"><Carousel shape="bar" placement="bottom" style={{ height: 120 }}><div>Slide 1</div><div>Slide 2</div><div>Slide 3</div></Carousel></div>
  ...
  if (name === "Segmented") return <div className="demo-row"><SegmentedControl defaultValue="周" data={["日", "周", "月"].map((label) => ({ label, value: label }))} /><SegmentedControl block defaultValue="A" data={["A", "B"].map((label) => ({ label, value: label }))} disabled /></div>
