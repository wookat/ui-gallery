...
export function DashboardPage() {
  ...
  return <div class="space-y-6">
    ...
    <Show when={ready()} fallback={<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><For each={[1, 2, 3, 4]}>{() => <Card><CardHeader><Skeleton class="h-4 w-24" /><Skeleton class="h-8 w-32" /></CardHeader><CardContent><Skeleton class="h-12 w-full" /></CardContent></Card>}</For></div>}>
      ...
      <div class="grid gap-4 xl:grid-cols-3"><Card><CardHeader><CardTitle>订单趋势</CardTitle></CardHeader><CardContent><div class="h-56"><ChartCanvas config={barConfig} /></div></CardContent></Card><Card class="xl:col-span-2"><CardHeader><CardTitle>最近订单</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>订单</TableHead><TableHead>客户</TableHead><TableHead>产品</TableHead><TableHead>状态</TableHead><TableHead class="text-right">金额</TableHead><TableHead class="w-12" /></TableRow></TableHeader><TableBody><For each={orders.slice(0, 5)}>{(order) => <TableRow><TableCell>{order.id}</TableCell><TableCell><div class="flex items-center gap-2"><Avatar name={order.customer} />{order.customer}</div></TableCell><TableCell>{order.product}</TableCell><TableCell><StatusBadge value={order.status} /></TableCell><TableCell class="text-right tabular-nums">{money(order.amount)}</TableCell><TableCell><DropdownMenu.Root><DropdownMenu.Trigger as={Button} variant="ghost" size="icon" aria-label="订单操作">⋯</DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content class="z-50 rounded-md border border-zinc-200 bg-white p-1 shadow-lg dark:border-zinc-700 dark:bg-zinc-900"><DropdownMenu.Item class="min-h-10 px-3 py-2 text-sm">查看</DropdownMenu.Item><DropdownMenu.Item class="min-h-10 px-3 py-2 text-sm">编辑</DropdownMenu.Item><DropdownMenu.Item class="min-h-10 px-3 py-2 text-sm text-red-600 dark:text-red-400">删除</DropdownMenu.Item></DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root></TableCell></TableRow>}</For></TableBody></Table></CardContent></Card></div>
