...
export function OrdersPage() {
  ...
  const filtered = createMemo(() => {
    ...
    return rows()
      .filter((order) => order.id.toLowerCase().includes(query().toLowerCase()) && (status() === "all" || order.status === status()) && (channelValue === "all" || (Array.isArray(channelValue) && channelValue.includes(order.channel))))
  ...
  const remove = () => { const item = selected(); if (item) setRows((items) => items.filter((row) => row.id !== item.id)); setSelected(null); setDeleteOpen(false); toast.success("订单已删除") }
  return <div class="space-y-6">
    ...
    <Card><CardContent class="space-y-4 pt-5">
      ...
      <Show when={state() !== "loading"} fallback={<div class="grid gap-3"><For each={[1, 2, 3, 4, 5, 6]}>{() => <Skeleton class="h-10 w-full" />}</For></div>}>
        <Show when={state() !== "empty" && visible().length > 0} fallback={<div class="grid place-items-center gap-3 py-16 text-center"><Icon name="inbox" size={36} class="text-zinc-400" /><p class="font-medium">没有找到订单</p><p class="text-sm text-zinc-500">调整搜索或筛选条件后重试。</p><Button variant="outline" onClick={() => { setQuery(""); setStatus("all") }}>清除筛选</Button></div>}>
