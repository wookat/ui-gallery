 ...
 * 命名宽度（w-popover / w-sheet / w-sidebar-drawer …）、min-h-hit、pr-hit / pl-hit（为 ✕ / 前置图标预留热区）
 * 另在 w / min-h / pr / pl 组显式登记一次（chat 与 settings 侧分别引入，取并集），调用方 pr-0 才能真正去掉组件默认的右留白。
...
export const cn = createCn({
  extend: {
    classGroups: {
      ...
      "min-h": [{ "min-h": [(v: string) => /^[a-z][a-z-]*$/.test(v)] }],
      pr: [{ pr: [(v: string) => /^[a-z][a-z-]*$/.test(v)] }],
      pl: [{ pl: [(v: string) => /^[a-z][a-z-]*$/.test(v)] }],
    },
