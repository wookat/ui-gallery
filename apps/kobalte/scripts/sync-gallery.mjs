...
const source = readFileSync(`${root}apps/kobalte/src/coverage.ts`, "utf8")
const galleryPath = `${root}apps/kobalte/gallery.json`
const gallery = JSON.parse(readFileSync(galleryPath, "utf8"))
const entries = Object.fromEntries(
  [
    ...source.matchAll(
      /\b([A-Za-z0-9]+): "(implemented|composed|missing)"/g,
    ),
  ].map((match) => [match[1], match[2]]),
)
gallery.coverage = Object.fromEntries(
  contract.components.map((name) => [name, entries[name]]),
)
