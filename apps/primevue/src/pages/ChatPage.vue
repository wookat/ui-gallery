...
type Message = { role: string; content: string; sources?: string[]; tool?: { name: string; args: Record<string, string>; status: string }; streaming?: boolean }
...
<template>
  <div class="chat-page">
    ...
    <div class="chat-layout">
      <aside class="conversation-sidebar desktop-only"><div class="flex items-center justify-between gap-2 mb-3"><strong>会话</strong><Button label="新建" icon="pi pi-plus" size="small" @click="createConversation" /></div><IconField class="w-full mb-3"><InputIcon class="pi pi-search" /><InputText v-model="conversationSearch" placeholder="搜索会话" fluid /></IconField><div class="conversation-list"><button v-for="item in filteredConversations" :key="item.id" class="conversation-item" :class="{ active: item.id === activeConversation }" @click="activeConversation = item.id"><span class="min-w-0 flex-1"><strong class="truncate">{{ item.title }}</strong><small>{{ item.time }}</small></span><Badge v-if="item.unread" :value="item.unread" severity="danger" /></button></div></aside>
      <Drawer v-model:visible="mobileOpen" header="会话" class="mobile-conversations" position="left"><Button label="新建会话" icon="pi pi-plus" fluid outlined @click="createConversation(); mobileOpen = false" /><div class="conversation-list mt-3"><button v-for="item in filteredConversations" :key="item.id" class="conversation-item" :class="{ active: item.id === activeConversation }" @click="activeConversation = item.id; mobileOpen = false"><span class="min-w-0 flex-1"><strong class="truncate">{{ item.title }}</strong><small>{{ item.time }}</small></span><Badge v-if="item.unread" :value="item.unread" severity="danger" /></button></div></Drawer>
...
.message-stream { flex: 1; overflow-y: auto; padding: 24px; display: flex; flex-direction: column; gap: 20px; } .message-row { display: flex; align-items: flex-start; gap: 10px; max-width: min(780px, 100%); } .message-row.user { margin-left: auto; flex-direction: row-reverse; } .message-content { min-width: 0; } .message-bubble { padding: 12px 14px; border: 1px solid var(--p-content-border-color); border-radius: 14px; background: var(--p-content-background); } .message-row.user .message-bubble { background: var(--p-highlight-background); }
.markdown :deep(p) { margin: 0 0 10px; } .markdown :deep(p:last-child) { margin-bottom: 0; } .markdown :deep(table) { width: 100%; border-collapse: collapse; margin-top: 10px; } .markdown :deep(th), .markdown :deep(td) { padding: 6px 8px; border: 1px solid var(--p-content-border-color); text-align: left; } .markdown :deep(code) { padding: 2px 4px; border-radius: 4px; background: var(--p-surface-100); } .code-wrap { position: relative; } .code-wrap pre { padding: 12px; padding-top: 34px; overflow: auto; background: var(--p-surface-900); color: var(--p-surface-0); border-radius: 8px; } .copy-code { position: absolute; top: 8px; right: 8px; border: 1px solid var(--p-surface-600); background: transparent; color: inherit; padding: 3px 8px; border-radius: 5px; cursor: pointer; }
