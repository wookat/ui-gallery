...
export function SettingsPage() {
  ...
  return (
    <Stack gap="lg">
      ...
      <Tabs defaultValue="profile" orientation="vertical" variant="pills" classNames={{ root: "ug-settings-tabs" }}>
        <Tabs.List miw={180}>
                ...
                {sessions.map((s) => (
                  <List.Item key={s.device}>
                    <Group justify="space-between" wrap="nowrap">
                      <Group gap="sm" wrap="nowrap"><ThemeIcon variant="light" color="gray"><Icon name={s.device.includes("iPhone") ? "smartphone" : "monitor"} size={16} /></ThemeIcon><div><Text size="sm" fw={500}>{s.device} {s.current ? <Badge size="xs" color="green" ml={4}>当前</Badge> : null}</Text><Text size="xs" c="dimmed">{s.location} · {s.time}</Text></div></Group>
                ...
                <Table.Tbody>
                  {team.map((m) => (
                    <Table.Tr key={m.email}>
                      <Table.Td><Group gap="sm" wrap="nowrap"><Avatar radius="xl" size="sm" color="blue">{m.name.slice(0, 1)}</Avatar><div><Text size="sm">{m.name}</Text><Text size="xs" c="dimmed">{m.email}</Text></div></Group></Table.Td>
                      <Table.Td><Select size="xs" w={120} data={roleOptions} defaultValue={m.role} allowDeselect={false} disabled={m.role === "owner"} /></Table.Td>
                      <Table.Td><Text size="sm" c="dimmed">{m.lastActive}</Text></Table.Td>
                      <Table.Td ta="right"><ActionIcon variant="subtle" color="red" aria-label="移除" disabled={m.role === "owner"}><Icon name="trash" size={15} /></ActionIcon></Table.Td>
                    </Table.Tr>
                  ))}
                </Table.Tbody>
              </Table>
            ...
            <SimpleGrid cols={{ base: 1, md: 3 }}>
              {plans.map((p) => (
                <Card key={p.name} withBorder radius="md" padding="lg" style={p.recommended ? { borderColor: "var(--mantine-primary-color-filled)" } : undefined}>
                  ...
                  <Text fz={28} fw={700} mt="sm">{p.price === null ? "联系我们" : p.price === 0 ? "免费" : money(yearly ? p.price * 12 * 0.8 : p.price)}{p.price ? <Text span size="sm" c="dimmed" fw={400}> /{yearly ? "年" : "月"}</Text> : null}</Text>
                  ...
                  <Table.Tbody>
                    {invoices.map((inv) => (
                      <Table.Tr key={inv.id}>
                        ...
                        <Table.Td ta="right"><ActionIcon variant="subtle" color="gray" aria-label="下载"><Icon name="download" size={15} /></ActionIcon></Table.Td>
                      </Table.Tr>
                    ))}
                  </Table.Tbody>
                </Table>
      ...
      <Card withBorder radius="md" padding="lg" style={{ borderColor: "var(--mantine-color-red-filled)" }}>
        <Group justify="space-between" wrap="wrap">
          <div><Title order={5} c="red">危险区</Title><Text size="sm" c="dimmed">删除账号将永久移除所有数据，不可恢复。</Text></div>
