...
@Component({
  ...
  template: `
    <main class="auth-page">
      ...
      <section class="auth-panel"><div class="auth-card"><div class="mobile-auth-brand"><a class="brand" routerLink="/landing"><span class="brand-mark"><mat-icon svgIcon="sparkles"></mat-icon></span>Acme Console</a></div><div class="auth-heading"><p class="eyebrow">WELCOME BACK</p><h1>登录你的工作台</h1><p class="muted">继续你的工作，保持一切井然有序。</p></div>@if (showError()) {<div class="auth-alert"><mat-icon svgIcon="error"></mat-icon><span>邮箱或密码不正确，请重试。</span><button mat-icon-button (click)="showError.set(false)" aria-label="关闭提示"><mat-icon svgIcon="x"></mat-icon></button></div>}<form [formGroup]="loginForm" (ngSubmit)="submit()" class="stack"><mat-form-field appearance="outline"><mat-label>邮箱</mat-label><mat-icon matPrefix svgIcon="mail"></mat-icon><input matInput type="email" formControlName="email" autocomplete="email"><mat-error>请输入有效的邮箱</mat-error></mat-form-field><mat-form-field appearance="outline"><mat-label>密码</mat-label><mat-icon matPrefix svgIcon="lock"></mat-icon><input matInput [type]="passwordVisible() ? 'text' : 'password'" formControlName="password" autocomplete="current-password"><button mat-icon-button matSuffix type="button" (click)="passwordVisible.set(!passwordVisible())" [attr.aria-label]="passwordVisible() ? '隐藏密码' : '显示密码'"><mat-icon [svgIcon]="passwordVisible() ? 'eye-off' : 'eye'"></mat-icon></button><mat-error>请输入密码</mat-error></mat-form-field><div class="spread"><mat-checkbox formControlName="remember">记住我</mat-checkbox><a routerLink="/login">忘记密码？</a></div><button mat-flat-button color="primary" class="full" type="submit" [disabled]="submitting()"><span class="btn-content">@if (submitting()) {<mat-spinner diameter="18"></mat-spinner>登录中…} @else {登录 <mat-icon iconPositionEnd svgIcon="arrow-right"></mat-icon>}</span></button></form><div class="or-divider"><span>或</span></div><div class="oauth-grid"><button mat-stroked-button type="button" (click)="oauth('Google')"><mat-icon svgIcon="globe"></mat-icon>Google</button><button mat-stroked-button type="button" (click)="oauth('GitHub')"><mat-icon svgIcon="code"></mat-icon>GitHub</button><button mat-stroked-button type="button" (click)="oauth('WeChat')"><mat-icon svgIcon="message-square"></mat-icon>微信</button></div><p class="auth-switch muted">还没有账户？ <a routerLink="/login">免费注册</a></p><small class="muted auth-terms">继续即表示你同意服务条款和隐私政策。</small></div></section>
    </main>
  `,
})
