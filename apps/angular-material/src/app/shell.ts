...
@Component({
  selector: 'app-shell',
  standalone: true,
  imports: [NgFor, NgIf, RouterOutlet, RouterLink, RouterLinkActive, MatBadgeModule, MatButtonModule, MatIconModule, MatInputModule, MatListModule, MatMenuModule, MatSidenavModule, MatToolbarModule],
  template: `
    <mat-sidenav-container class="shell-container">
      <mat-sidenav #sidenav [mode]="mobile() ? 'over' : 'side'" [opened]="!mobile() || drawerOpen()" class="shell-sidenav">
        <div class="brand"><span class="brand-mark">A</span><span *ngIf="!collapsed()">Acme Console</span></div>
        ...
        <mat-nav-list>
          <a mat-list-item *ngFor="let item of navItems" [routerLink]="item.path" routerLinkActive="active" [routerLinkActiveOptions]="{exact: item.path === '/'}" queryParamsHandling="preserve" (click)="mobile() && drawerOpen.set(false)">
            <mat-icon matListItemIcon [svgIcon]="item.icon"></mat-icon><span matListItemTitle *ngIf="!collapsed()">{{ item.label }}</span>
            <span *ngIf="item.badge && !collapsed()" matBadge="{{item.badge}}" matBadgeSize="small" matBadgeColor="accent"></span>
          </a>
        </mat-nav-list>
        <div class="sidenav-user" *ngIf="!collapsed()"><span class="avatar">林</span><span><b>林晓</b><small>管理员</small></span><button mat-icon-button [matMenuTriggerFor]="userMenu"><mat-icon svgIcon="more"></mat-icon></button></div>
      </mat-sidenav>
      <mat-sidenav-content>
        <mat-toolbar class="topbar">
          <button mat-icon-button *ngIf="mobile()" (click)="drawerOpen.set(true)"><mat-icon svgIcon="menu"></mat-icon></button>
          <span class="breadcrumb">Acme Console <span>/</span> {{ currentLabel() }}</span><span class="topbar-spacer"></span>
          <mat-form-field class="search-field" appearance="outline"><mat-icon matPrefix svgIcon="search"></mat-icon><input matInput placeholder="搜索..." /></mat-form-field>
          <button mat-icon-button [matMenuTriggerFor]="noticeMenu" matBadge="2" matBadgeSize="small" matBadgeColor="accent" aria-label="通知"><mat-icon svgIcon="bell"></mat-icon></button>
    ...
    <mat-menu #noticeMenu="matMenu"><div class="menu-title">通知</div><button mat-menu-item *ngFor="let notice of notices"><span>{{ notice.title }}</span><small>{{ notice.time }}</small></button></mat-menu>
  ...
  `,
  styles: [`
    :host { display:block; min-height:100vh; }.shell-container { min-height:100vh; }.shell-sidenav { width:260px; border-right:1px solid var(--mat-sys-outline-variant); }
    ...
    @media (max-width:959px) { .search-field { display:none; }.shell-sidenav { width:280px; } }
  `],
})
export class ShellComponent {
  readonly navItems = nav; readonly notices = notifications; readonly collapsed = signal(false); readonly drawerOpen = signal(false); readonly mobile = signal(false);
