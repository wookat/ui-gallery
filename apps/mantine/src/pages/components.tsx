  ...
  return (
    <Card withBorder radius="md" padding="md" id={`c-${name}`} style={{ scrollMarginTop: 80 }}>
      <Group justify="space-between" mb="sm" wrap="wrap">
        <Group gap="xs"><Text fw={600} size="sm">{name}</Text>{mantine ? <Text size="xs" c="dimmed">{mantine}</Text> : null}</Group>
        <Badge size="xs" variant="light" color={covColor[c]}>{c}</Badge>
      </Group>
      {c === "missing" ? <Text size="sm" c="dimmed">Mantine 未提供对应组件（未引入第三方替代）。</Text> : children}
    </Card>
  )
}
    ...
    <Stack gap="xl">
      ...
      <Stack gap="md" id="typography" style={{ scrollMarginTop: 80 }}>
        ...
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          ...
          <Demo name="Link" mantine="Anchor"><Group>{sizes.map((s) => <Anchor key={s} href="#" size={s}>链接 {s}</Anchor>)}<Anchor href="#" underline="always">总是下划线</Anchor><Anchor href="#" underline="never" c="dimmed">无下划线</Anchor></Group></Demo>
        </SimpleGrid>
      </Stack>
          ...
          <Stack gap="sm">
            {buttonVariants.map((v) => (
              <Group key={v} gap="xs" wrap="wrap" align="center">
                <Text size="xs" c="dimmed" w={80}>{v}</Text>
      ...
      <Stack gap="md" id="inputs" style={{ scrollMarginTop: 80 }}>
        ...
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          ...
          <Demo name="Upload" mantine="@mantine/dropzone Dropzone / FileInput"><Stack gap="sm"><FileInput placeholder="选择文件" leftSection={<Icon name="paperclip" size={15} />} clearable /><FileInput multiple placeholder="多文件" /><Dropzone onDrop={() => {}} radius="md" mih={100}><Center h={80}><Group gap="sm" c="dimmed"><Icon name="upload" size={24} /><Text size="sm">拖拽文件到此处</Text></Group></Center></Dropzone><Dropzone onDrop={() => {}} loading mih={60}><Center h={40}><Text size="sm">上传中</Text></Center></Dropzone><Dropzone onDrop={() => {}} disabled mih={60}><Center h={40}><Text size="sm" c="dimmed">禁用</Text></Center></Dropzone></Stack></Demo>
      ...
      <Stack gap="md" id="data" style={{ scrollMarginTop: 80 }}>
        ...
        <Demo name="DataGrid" mantine="Table + 排序/选择/分页（见 /orders 页）"><Text size="sm" c="dimmed">Mantine 核心不含 DataGrid；/orders 页面用 Table + Checkbox + Pagination + Menu 组合实现排序、多选、列开关与分页。</Text><Group mt="sm"><Button component="a" href="/apps/mantine/orders" variant="light" size="xs">查看 /orders</Button></Group></Demo>
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          ...
          <Demo name="List" mantine="List / NavLink"><List spacing="xs" size="sm" icon={<ThemeIcon size={20} radius="xl" variant="light"><Icon name="check" size={12} /></ThemeIcon>}>{team.slice(0, 3).map((m) => <List.Item key={m.email}>{m.name} · {m.role}</List.Item>)}</List><Paper withBorder mt="md" p={4}>{team.slice(0, 3).map((m, i) => <NavLink key={m.email} label={m.name} description={m.email} active={i === 0} leftSection={<Avatar size="sm" radius="xl" color="initials" name={m.name} />} rightSection={<Badge size="xs" variant="light">{m.role}</Badge>} />)}</Paper></Demo>
          <Demo name="Card" mantine="Card / Paper"><Stack gap="sm"><Card withBorder radius="md" padding="lg"><Card.Section><Image src={null} h={100} fallbackSrc="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg'/%3E" alt="占位" bg="var(--mantine-color-default)" /></Card.Section><Text fw={600} mt="md">图片卡片</Text><Text size="sm" c="dimmed">带 Card.Section 与描述文字。</Text><Button fullWidth mt="md" variant="light">操作</Button></Card><Card shadow="sm" radius="md" padding="md"><Group><ThemeIcon variant="light"><Icon name="zap" size={16} /></ThemeIcon><div><Text fw={600} size="sm">横向卡片</Text><Text size="xs" c="dimmed">shadow=sm</Text></div></Group></Card><Paper withBorder p="md" radius="md">Paper 容器</Paper></Stack></Demo>
          <Demo name="Avatar" mantine="Avatar / Indicator"><Group>{sizes.map((s) => <Avatar key={s} size={s} radius="xl" color="initials" name={team[0].name} />)}<Avatar radius="sm" color="blue"><Icon name="user" size={16} /></Avatar><Avatar variant="outline" color="teal">AC</Avatar><Indicator color="green" size={10} offset={4} processing><Avatar radius="xl" color="blue">在</Avatar></Indicator></Group></Demo>
          <Demo name="AvatarGroup" mantine="Avatar.Group"><Avatar.Group spacing="sm">{team.slice(0, 4).map((m) => <Avatar key={m.email} radius="xl" color="initials" name={m.name} />)}<Avatar radius="xl">+{team.length - 4}</Avatar></Avatar.Group></Demo>
          ...
          <Demo name="Statistic" mantine="Text + NumberFormatter + Badge（组合）"><SimpleGrid cols={2}>{stats.slice(0, 2).map((s) => <Paper key={s.label} withBorder p="md" radius="md"><Text size="xs" c="dimmed" tt="uppercase">{s.label}</Text><Text fz={24} fw={700}><NumberFormatter value={s.value} prefix={s.unit === "CNY" ? "¥" : ""} suffix={s.unit === "%" ? "%" : ""} thousandSeparator /></Text><Badge size="xs" color={s.delta >= 0 ? "green" : "red"} variant="light">{s.delta >= 0 ? "+" : ""}{s.delta}%</Badge></Paper>)}</SimpleGrid></Demo>
          <Demo name="Timeline" mantine="Timeline"><Timeline active={1} bulletSize={22} lineWidth={2}>{activity.slice(0, 4).map((a, i) => <Timeline.Item key={i} bullet={<Icon name="check" size={12} />} title={a.user}><Text size="sm" c="dimmed">{a.action}</Text><Text size="xs" mt={4}>{a.time}</Text></Timeline.Item>)}</Timeline></Demo>
          ...
          <Demo name="Image" mantine="Image / AspectRatio + Modal 预览"><Stack gap="sm"><Image radius="md" h={120} src={null} fallbackSrc="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg'/%3E" alt="占位" bg="var(--mantine-color-default)" /><Button size="xs" variant="light" onClick={() => setModal("basic")}>预览</Button></Stack></Demo>
          <Demo name="Carousel" mantine="@mantine/carousel"><Carousel withIndicators height={120} slideSize="50%" slideGap="sm" emblaOptions={{ loop: true }}>{[1, 2, 3, 4].map((i) => <Carousel.Slide key={i}><Center h="100%" style={{ borderRadius: 8, background: "var(--mantine-color-default)" }}><Text>Slide {i}</Text></Center></Carousel.Slide>)}</Carousel></Demo>
      ...
      <Stack gap="md" id="feedback" style={{ scrollMarginTop: 80 }}>
        ...
        <Demo name="Alert" mantine="Alert — 4 级 × 4 variant"><Stack gap="sm">{[["blue", "信息", "info"], ["green", "成功", "check"], ["yellow", "警告", "alert-triangle"], ["red", "错误", "alert-circle"]].map(([c, t, i]) => <Group key={c} grow align="stretch">{(["light", "filled", "outline", "white"] as const).map((v) => <Alert key={v} color={c} variant={v} title={`${t} · ${v}`} icon={<Icon name={i} size={16} />} withCloseButton={v === "light"}>提示内容。</Alert>)}</Group>)}</Stack></Demo>
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          ...
          <Demo name="Result" mantine="ThemeIcon + Title + Text + Button（组合）"><Stack align="center" gap="xs" py="sm"><ThemeIcon size={48} radius="xl" color="teal" variant="light"><Icon name="check" size={24} /></ThemeIcon><Title order={4}>操作成功</Title><Text size="sm" c="dimmed">项目已创建。</Text><Group><Button size="xs" variant="default">返回</Button><Button size="xs">继续</Button></Group></Stack></Demo>
          <Demo name="Popconfirm" mantine="Popover + Button（组合）"><Popover opened={popconfirm} onChange={setPopconfirm} width={220} withArrow position="top"><Popover.Target><Button size="xs" color="red" variant="light" onClick={() => setPopconfirm((v) => !v)}>删除</Button></Popover.Target><Popover.Dropdown><Group gap="xs" wrap="nowrap" align="flex-start"><Icon name="alert-circle" size={16} /><Text size="sm">确定删除此项？</Text></Group><Group justify="flex-end" gap="xs" mt="sm"><Button size="compact-xs" variant="default" onClick={() => setPopconfirm(false)}>取消</Button><Button size="compact-xs" color="red" onClick={() => { setPopconfirm(false); toast("red", "已删除") }}>确定</Button></Group></Popover.Dropdown></Popover></Demo>
        </SimpleGrid>
      </Stack>
      ...
      <Stack gap="md" id="navigation" style={{ scrollMarginTop: 80 }}>
        ...
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          ...
          <Demo name="BackTop" mantine="Affix + Transition + useWindowScroll（组合）"><Text size="sm" c="dimmed">向下滚动后右下角出现「回到顶部」按钮。</Text></Demo>
          <Demo name="Affix" mantine="Affix"><Text size="sm" c="dimmed">Affix 固定定位：右下角悬浮按钮即为 Affix 渲染。</Text></Demo>
      ...
      <Stack gap="md" id="layout" style={{ scrollMarginTop: 80 }}>
        ...
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          <Demo name="Grid" mantine="Grid / SimpleGrid"><Grid gap="xs">{[12, 6, 6, 4, 4, 4, 3, 3, 3, 3].map((span, i) => <Grid.Col key={i} span={{ base: 12, xs: span }}><Center h={32} bg="var(--mantine-color-default)" style={{ borderRadius: 4 }}><Text size="xs">{span}</Text></Center></Grid.Col>)}</Grid></Demo>
          <Demo name="Stack" mantine="Stack / Group / Flex / Space"><Stack gap="xs">{[1, 2].map((i) => <Center key={i} h={28} bg="var(--mantine-color-default)" style={{ borderRadius: 4 }}><Text size="xs">Stack {i}</Text></Center>)}</Stack><Space h="md" /><Group grow>{[1, 2, 3].map((i) => <Center key={i} h={28} bg="var(--mantine-color-default)" style={{ borderRadius: 4 }}><Text size="xs">Group {i}</Text></Center>)}</Group><Space h="md" /><Flex gap="xs" wrap="wrap" justify="space-between">{[1, 2, 3].map((i) => <Center key={i} h={28} w={80} bg="var(--mantine-color-default)" style={{ borderRadius: 4 }}><Text size="xs">Flex {i}</Text></Center>)}</Flex></Demo>
          <Demo name="Layout" mantine="AppShell（本页外层即 AppShell）"><Paper withBorder h={140} style={{ overflow: "hidden" }}><Stack gap={0} h="100%"><Center h={28} bg="var(--mantine-color-default)"><Text size="xs">Header</Text></Center><Flex h={112}><Center w={60} bg="var(--mantine-color-default)"><Text size="xs">Nav</Text></Center><Center style={{ flex: 1 }}><Text size="xs">Main</Text></Center></Flex></Stack></Paper></Demo>
          <Demo name="Container" mantine="Container"><Stack gap="xs">{(["xs", "sm", "md"] as const).map((s) => <Container key={s} size={s} w="100%" px={0}><Center h={28} bg="var(--mantine-color-default)" style={{ borderRadius: 4 }}><Text size="xs">Container {s}</Text></Center></Container>)}</Stack></Demo>
          <Demo name="AspectRatio" mantine="AspectRatio"><Group grow><AspectRatio ratio={16 / 9}><Center bg="var(--mantine-color-default)" style={{ borderRadius: 8 }}><Text size="xs">16:9</Text></Center></AspectRatio><AspectRatio ratio={1}><Center bg="var(--mantine-color-default)" style={{ borderRadius: 8 }}><Text size="xs">1:1</Text></Center></AspectRatio></Group></Demo>
          <Demo name="Resizable" mantine="Splitter"><Splitter style={{ height: 120 }}><Splitter.Pane defaultSize="50%" min="20%"><Center h="100%" bg="var(--mantine-color-default)"><Text size="xs">左侧面板</Text></Center></Splitter.Pane><Splitter.Pane defaultSize="50%" min="20%"><Center h="100%"><Text size="xs">右侧面板</Text></Center></Splitter.Pane></Splitter></Demo>
          ...
          <Demo name="Accordion" mantine="Accordion / Collapse"><Stack gap="sm">{(["default", "contained", "filled", "separated"] as const).map((v) => <Accordion key={v} variant={v} defaultValue="a"><Accordion.Item value="a"><Accordion.Control icon={<Icon name="info" size={14} />}>{v} 面板</Accordion.Control><Accordion.Panel><Text size="sm">内容</Text></Accordion.Panel></Accordion.Item><Accordion.Item value="b"><Accordion.Control>第二项</Accordion.Control><Accordion.Panel><Text size="sm">内容 2</Text></Accordion.Panel></Accordion.Item></Accordion>)}<Collapse expanded><Text size="sm" c="dimmed">Collapse 已展开</Text></Collapse></Stack></Demo>
        </SimpleGrid>
      </Stack>
      ...
      <Stack gap="md" id="other" style={{ scrollMarginTop: 80 }}>
        ...
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          <Demo name="ThemeProvider" mantine="MantineProvider / useMantineColorScheme"><Group><SegmentedControl value={colorScheme} onChange={(v) => setColorScheme(v as "light" | "dark" | "auto")} data={[{ value: "light", label: "浅色" }, { value: "dark", label: "深色" }, { value: "auto", label: "自动" }]} /><Group gap={4}>{colors.map((c) => <Button key={c} size="compact-xs" color={c}>{c}</Button>)}</Group></Group><Text size="xs" c="dimmed" mt="sm">默认主题：primaryColor blue，未自定义配色；URL ?theme= 控制初始 colorScheme。</Text></Demo>
          <Demo name="Watermark" />
          <Demo name="Tour" />
          <Demo name="FloatButton" mantine="Affix + ActionIcon（组合）"><Text size="sm" c="dimmed">右下角固定的圆形操作按钮（Affix 渲染）。</Text></Demo>
        </SimpleGrid>
      </Stack>
