...
@Component({
  ...
  template: `
    <section class="components-page">
      ...
      <nz-card class="index-card" id="component-index">
        <div class="index-head"><h2 nz-typography nzType="secondary">组件索引（{{ components.length }}）</h2><button nz-button nzType="link" nzSize="small" class="index-toggle" (click)="indexExpanded = !indexExpanded">{{ indexExpanded ? '收起' : '展开全部' }}</button></div>
        <div class="index" [class.index-collapsed]="!indexExpanded">@for (name of components; track name) { <a [href]="'#comp-' + name">{{ name }}</a> }</div>
      </nz-card>
      <div class="sections">@for (name of components; track name) {
        <section class="component-section" [id]="'comp-' + name">
          ...
          <nz-card nzSize="small">
            @switch (name) {
              @case ('Typography') { <h1 nz-typography>一级标题</h1><h2 nz-typography>二级标题</h2><h3 nz-typography>三级标题</h3><h4 nz-typography>四级标题</h4><h5 nz-typography>五级标题</h5><h6 nz-typography class="h6">六级标题</h6><p nz-typography>正文，内联 <code>code</code> 与 <kbd>Ctrl</kbd> + <kbd>K</kbd> 快捷键。</p><blockquote nz-typography>引用文字</blockquote><ul nz-typography><li>列表项一</li><li>列表项二</li></ul><ol nz-typography><li>有序一</li><li>有序二</li></ol><p nz-typography nzType="secondary">辅助说明</p><a nz-typography href="#comp-Link">链接</a> }
              @case ('Button') { <div class="demo-row"><button nz-button nzType="primary">主要</button><button nz-button>默认</button><button nz-button nzType="dashed">虚线</button><button nz-button nzType="text">文本</button><button nz-button nzType="link">链接</button><button nz-button nzDanger>危险</button><button nz-button nzType="primary" nzLoading>加载中</button><button nz-button nzType="primary" disabled>禁用</button><button nz-button disabled>禁用</button><button nz-button nzType="primary" nzLoading disabled>加载禁用</button></div><div class="demo-row"><button nz-button nzSize="small">小</button><button nz-button>默认</button><button nz-button nzSize="large">大</button><button nz-button nzShape="circle"><ui-icon name="plus" /></button></div> }
              ...
              @case ('NumberInput') { <nz-input-number nzSize="small" [nzMin]="0" [nzMax]="100" [nzStep]="1" [(ngModel)]="numberValue" /><nz-input-number [nzMin]="0" [nzMax]="100" [nzStep]="1" [(ngModel)]="numberValue" /><nz-input-number nzSize="large" nzStatus="error" [nzMin]="0" [ngModel]="120" /><nz-input-number [nzMin]="0" [ngModel]="8" nzDisabled /> }
              @case ('Select') { <div class="demo-row"><nz-select nzSize="small" [(ngModel)]="selectValue" nzPlaceHolder="小尺寸"><nz-option nzValue="all" nzLabel="全部" /></nz-select><nz-select [(ngModel)]="selectValue" nzPlaceHolder="选择一项"><nz-option nzValue="all" nzLabel="全部" /><nz-option nzValue="paid" nzLabel="已支付" /></nz-select><nz-select nzSize="large" nzStatus="error" [ngModel]="'error'"><nz-option nzValue="error" nzLabel="校验失败" /></nz-select><nz-select nzDisabled [ngModel]="'disabled'"><nz-option nzValue="disabled" nzLabel="禁用" /></nz-select></div> }
              ...
              @case ('Form') { <div class="stacked-demo">
                <p nz-typography nzType="secondary">horizontal</p>
                <form nz-form class="demo-form" nzLayout="horizontal"><nz-form-item><nz-form-label [nzSpan]="6" nzRequired>名称</nz-form-label><nz-form-control [nzSpan]="18" nzErrorTip="请输入名称"><input nz-input required /></nz-form-control></nz-form-item><nz-form-item><nz-form-label [nzSpan]="6">邮箱</nz-form-label><nz-form-control [nzSpan]="18" nzValidateStatus="error" nzErrorTip="邮箱格式不正确"><input nz-input type="email" nzStatus="error" value="invalid" /></nz-form-control></nz-form-item></form>
                <p nz-typography nzType="secondary">vertical</p>
                <form nz-form class="demo-form" nzLayout="vertical"><nz-form-item><nz-form-label nzRequired>名称</nz-form-label><nz-form-control><input nz-input /></nz-form-control></nz-form-item><nz-form-item><nz-form-label>备注</nz-form-label><nz-form-control><textarea nz-input rows="2"></textarea></nz-form-control></nz-form-item></form>
                <p nz-typography nzType="secondary">inline</p>
                <form nz-form nzLayout="inline"><nz-form-item><nz-form-label>关键字</nz-form-label><nz-form-control><input nz-input /></nz-form-control></nz-form-item><nz-form-item><nz-form-control><button nz-button nzType="primary">搜索</button></nz-form-control></nz-form-item></form>
              </div> }
              @case ('Table') { <nz-table #table [nzData]="rows" [nzShowPagination]="false" [nzScroll]="viewport.isMobile() ? {} : {x:'640px'}"><thead><tr><th>订单</th><th>客户</th><th>状态</th><th>金额</th></tr></thead><tbody>@for(row of table.data;track row.id){<tr><td>{{row.id}}</td><td>{{row.customer}}</td><td><nz-tag nzColor="success">已支付</nz-tag></td><td>¥ {{row.amount}}</td></tr>}</tbody></nz-table> }
              @case ('DataGrid') { <nz-table #dataGrid [nzData]="rows" nzSize="small" [nzLoading]="false" [nzShowPagination]="false" [nzScroll]="viewport.isMobile() ? {} : {x:'720px'}"><thead><tr><th [nzLeft]="!viewport.isMobile()" [nzWidth]="viewport.isMobile() ? '40px' : '120px'"><label nz-checkbox [nzChecked]="allRowsSelected" (nzCheckedChange)="toggleAllRows($event)"></label></th><th [nzLeft]="!viewport.isMobile()" [nzSortFn]="sortById">订单</th><th [nzFilters]="statusFilters" [nzFilterFn]="filterStatus">状态</th><th>客户</th><th>金额</th></tr></thead><tbody>@for(row of dataGrid.data;track row.id){<tr><td [nzLeft]="!viewport.isMobile()"><label nz-checkbox [ngModel]="isRowSelected(row.id)" (ngModelChange)="toggleRow(row.id, $event)"></label></td><td [nzLeft]="!viewport.isMobile()">{{row.id}}</td><td><nz-tag nzColor="success">已支付</nz-tag></td><td>{{row.customer}}</td><td>¥ {{row.amount}}</td></tr>}</tbody></nz-table> }
              ...
              @case ('Card') { <div class="card-demo"><nz-card nzSize="small" nzTitle="小卡片">卡片内容</nz-card><nz-card nzTitle="卡片标题" nzHoverable>卡片内容</nz-card><nz-card nzTitle="加载中" [nzLoading]="true">卡片内容</nz-card></div> }
              ...
              @case ('Progress') { <div class="progress-demo"><div class="progress-lines"><nz-progress nzSize="small" [nzPercent]="68" /><nz-progress [nzPercent]="82" nzStatus="success" /><nz-progress [nzPercent]="42" nzStatus="exception" /><nz-progress [nzPercent]="60" [nzSteps]="5" /></div><nz-progress nzType="circle" [nzPercent]="42" /></div> }
              ...
              @case ('Sidebar') { <nz-layout class="sidebar-demo"><nz-sider nzCollapsible [nzTrigger]="null" [nzTheme]="settings.theme"><ul nz-menu nzMode="inline" [nzTheme]="settings.theme"><li nz-menu-item nzSelected>概览</li><li nz-submenu nzTitle="管理"><ul><li nz-menu-item>订单</li><li nz-menu-item>客户</li></ul></li></ul></nz-sider><nz-content>内容区域</nz-content></nz-layout> }
              ...
              @case ('ThemeProvider') { <div class="theme-demo"><nz-switch [ngModel]="settings.theme === 'dark'" (ngModelChange)="toggleTheme()" /><span>当前主题：{{ settings.theme }}</span><p nz-typography nzType="secondary">主题由 UrlSettings 统一管理并同步到页面。</p></div> }
              @case ('Watermark') { <nz-watermark nzContent="Acme Console" [nzFont]="watermarkFont"><nz-card nzTitle="受保护内容">内部工作区</nz-card></nz-watermark> }
              ...
              @case ('Code') { <p nz-typography><code>const result = true;</code></p><pre nz-typography class="code-block"><code>const total = rows.length;</code></pre> }
              @case ('Kbd') { <p nz-typography><kbd>⌘</kbd> <kbd>K</kbd> 打开命令面板</p> }
              @case ('Link') { <a nz-typography href="#component-index">返回组件索引</a> <a nz-typography href="#comp-Button">跳转到 Button</a> }
...
export class ComponentsPage {
  ...
  rangeValue = [20, 70]; ratingValue = 4; colorValue = '#1890ff'; drawerOpen = false; drawerPlacement: 'top' | 'bottom' | 'left' | 'right' = 'right'; indexExpanded = false;
  readonly viewport = inject(Viewport);
  readonly watermarkFont = { color: 'rgba(128, 128, 128, 0.35)', fontSize: 16 };
