  ...
  <div class="space-y-6">
    ...
    <Card>
      ...
      <CardContent class="space-y-4">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span class="text-sm text-muted-foreground">显示 {{ visibleOrders.length }} / {{ filtered.length }} 条</span><div class="flex items-center gap-3"><Select v-model="pageSize" @update:model-value="resetPage"><SelectTrigger class="min-h-10 w-24"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="5">5 条</SelectItem><SelectItem value="10">10 条</SelectItem><SelectItem value="20">20 条</SelectItem></SelectContent></Select><Pagination v-model:page="page" :total="filtered.length" :items-per-page="Number(pageSize)"><PaginationContent v-slot="{ items }"><PaginationPrevious class="min-h-10 min-w-10" /><template v-for="item in items" :key="item.type === 'page' ? item.value : item.type"><PaginationItem v-if="item.type === 'page'" :value="item.value" class="min-h-10 min-w-10"><PaginationLink class="min-h-10 min-w-10" :is-active="item.value === page">{{ item.value }}</PaginationLink></PaginationItem></template><PaginationNext class="min-h-10 min-w-10" /></PaginationContent></Pagination></div></div>
      </CardContent>
    </Card>
