...
export function TableHead(props: ParentProps<{ class?: string; onClick?: () => void }>) { return <th class={`whitespace-nowrap px-3 py-3 text-left text-xs font-medium uppercase tracking-wide text-zinc-500 ${props.class ?? ""}`} onClick={() => props.onClick?.()}>{props.children}</th> }
export function TableCell(props: ParentProps<{ class?: string; onClick?: (event: MouseEvent) => void }>) { return <td class={`whitespace-nowrap px-3 py-3 ${props.class ?? ""}`} onClick={props.onClick}>{props.children}</td> }
