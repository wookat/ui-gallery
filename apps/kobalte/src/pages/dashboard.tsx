...
function Sparkline(props: { values: number[] }) {
  ...
  const points = props.values.map((value, index) => `${(index / (props.values.length - 1)) * 100},${40 - ((value - min) / Math.max(1, max - min)) * 34}`).join(" ")
  return <svg viewBox="0 0 100 40" class="h-12 w-full text-blue-600" aria-hidden="true"><polyline points={points} fill="none" stroke="currentColor" stroke-width="2" /></svg>
}
...
export function DashboardPage() {
  ...
  return <div class="space-y-6">
    ...
    <Show when={ready()} fallback={<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><For each={[1, 2, 3, 4]}>{() => <Card><CardHeader><Skeleton class="h-4 w-24" /><Skeleton class="h-8 w-32" /></CardHeader><CardContent><Skeleton class="h-12 w-full" /></CardContent></Card>}</For></div>}>
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><For each={stats}>{(item) => <Card><CardHeader class="pb-2"><div class="flex items-center justify-between text-sm text-zinc-500"><CardDescription>{item.label}</CardDescription><Badge variant={item.delta > 0 ? "default" : "destructive"}>{item.delta > 0 ? "↑" : "↓"} {Math.abs(item.delta)}%</Badge></div><CardTitle class="text-2xl">{item.unit === "CNY" ? money(item.value) : `${item.value}${item.unit ?? ""}`}</CardTitle></CardHeader><CardContent><Sparkline values={item.trend} /></CardContent></Card>}</For></div>
      <Tabs.Root defaultValue="month"><Tabs.List class="flex w-fit gap-1 rounded-md bg-zinc-100 p-1 dark:bg-zinc-800"><Tabs.Trigger value="day" class="rounded px-3 py-1.5 text-sm data-[selected]:bg-white dark:data-[selected]:bg-zinc-700">日</Tabs.Trigger><Tabs.Trigger value="week" class="rounded px-3 py-1.5 text-sm data-[selected]:bg-white dark:data-[selected]:bg-zinc-700">周</Tabs.Trigger><Tabs.Trigger value="month" class="rounded px-3 py-1.5 text-sm data-[selected]:bg-white dark:data-[selected]:bg-zinc-700">月</Tabs.Trigger></Tabs.List></Tabs.Root>
      ...
      <div class="grid gap-4 xl:grid-cols-2"><Card><CardHeader><CardTitle>团队动态</CardTitle></CardHeader><CardContent class="grid gap-4"><For each={activity}>{(item) => <div class="flex gap-3"><Avatar name={item.user} /><div class="min-w-0 flex-1 text-sm"><p><strong>{item.user}</strong> {item.action}</p><p class="text-xs text-zinc-500">{item.time}</p></div></div>}</For></CardContent></Card><Card><CardHeader><CardTitle>任务进度</CardTitle></CardHeader><CardContent class="grid gap-5"><For each={tasks}>{(task) => <Progress value={task.progress} label={task.title} />}</For></CardContent></Card></div>
    </Show>
  </div>
}
