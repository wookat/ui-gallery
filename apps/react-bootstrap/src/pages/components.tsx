...
import { Avatar, AvatarGroup, EmptyState, PageTitle, ResponsivePage, ResultView, Section } from "@/pages/shared"
...
function ComponentDemo({ name, openModal, openDrawer, openToast, openCommand }: { name: string; openModal: (mode: string) => void; openDrawer: (placement: "start" | "end" | "top" | "bottom") => void; openToast: () => void; openCommand: () => void }) {
  ...
  switch (name) {
    ...
    case "Card": return <Card><Card.Header>Header</Card.Header><Card.Body><Card.Title>Card title</Card.Title><CardSubtitle className="mb-2 text-secondary">Subtitle</CardSubtitle><Card.Text>带有 header、footer 和操作链接的卡片。</Card.Text><CardLink href="#component-Link">链接</CardLink></Card.Body><Card.Footer>Footer</Card.Footer></Card>
    ...
    case "Statistic": return <div><span className="text-secondary">本月收入</span><div className="display-6">¥128,430</div><Badge bg="success">+12.4%</Badge></div>
    ...
    case "Image": return <div className="d-flex flex-wrap gap-2"><Image src="https://placehold.co/100x70" fluid /><Image src="https://placehold.co/100x70" rounded /><Image src="https://placehold.co/100x70" roundedCircle /><Image src="https://placehold.co/100x70" thumbnail /></div>
    case "Carousel": return <Carousel indicators><CarouselItem><div className="bg-primary-subtle p-5 text-center">第一张 <Carousel.Caption><h3>标题</h3><p>带 captions 的 Carousel</p></Carousel.Caption></div></CarouselItem><CarouselItem><div className="bg-secondary-subtle p-5 text-center">第二张</div></CarouselItem></Carousel>
    ...
    case "Toast": case "Notification": return <Button onClick={openToast}>显示 Toast / Notification</Button>
    ...
    case "Drawer": case "Sidebar": return <div className="d-flex flex-wrap gap-2">{(["start", "end", "top", "bottom"] as const).map((placement) => <Button key={placement} variant="outline-primary" onClick={() => openDrawer(placement)}>打开 {placement} Drawer</Button>)}</div>
    ...
    case "BackTop": return <Button className="position-fixed bottom-0 end-0 m-4 rounded-circle" aria-label="返回顶部" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}><Icon name="arrow-up" /></Button>
...
function ButtonDemo() {
  ...
  return <><div className="d-flex flex-wrap gap-2 mb-2">{variants.map((variant) => <Button key={variant} variant={variant}>{variant}</Button>)}</div><div className="d-flex flex-wrap gap-2 mb-2">{variants.map((variant) => <Button key={variant} variant={`outline-${variant}`}>{`outline-${variant}`}</Button>)}</div><div className="d-flex flex-wrap gap-2 align-items-center"><Button size="sm">Small</Button><Button>Medium</Button><Button size="lg">Large</Button><Button active>Active</Button><Button disabled>Disabled</Button><Button><Spinner animation="border" size="sm" className="me-1" />Loading</Button><Button variant="outline-secondary" aria-label="图标按钮"><Icon name="plus" /></Button><Button variant="link">Link</Button></div></>
}
...
export function ComponentsPage() {
  ...
  const anchors = contractComponents.map((name) => <Nav.Link key={name} href={`#component-${name}`} className="p-0"><Badge bg="light" text="dark">{name}</Badge></Nav.Link>)
  return <ResponsivePage><PageTitle title="组件全集" subtitle="84 个 UI Gallery 合约组件、React Bootstrap 变体与组合示例。" /><Nav className="flex-wrap gap-2 mb-4">{anchors}</Nav>{Object.entries(groups).map(([group, names]) => <Section key={group} title={group}>{names.map((name) => <DemoCard key={name} name={name}><ComponentDemo name={name} openModal={setModal} openDrawer={setDrawer} openToast={() => setToast(true)} openCommand={() => setCommand(true)} /></DemoCard>)}</Section>)}<Section title="React Bootstrap 全部导出"><Card><Card.Body><p className="text-secondary">当前安装版本导出清单（组件导出均有清单或 live demo，hooks/contexts 以 Badge 列出）。</p><div className="d-flex flex-wrap gap-2">{rbExports.map((name) => <Badge key={name} bg={name.startsWith("use") || name.endsWith("Context") ? "dark" : "light"} text={name.startsWith("use") || name.endsWith("Context") ? "light" : "dark"}>{name}</Badge>)}</div><div className="d-flex flex-wrap gap-2 mt-3"><ButtonToolbar><ButtonGroup className="me-2"><ButtonToolbarDemo /></ButtonGroup></ButtonToolbar><CardGroupDemo /><FormFloatingDemo /><TabExportsDemo /><Button onClick={() => setCommand(true)}>打开导出 CommandPalette</Button></div><ExportExtras /></Card.Body></Card></Section><Modal show={Boolean(modal)} onHide={() => setModal(null)} centered={modal === "centered"} fullscreen={modal === "fullscreen" ? true : undefined} scrollable={modal === "scrollable"}><Modal.Header closeButton><Modal.Title>{modal === "confirm" ? "确认操作" : "Modal 示例"}</Modal.Title></Modal.Header><Modal.Body>{modal === "confirm" ? "确定要继续吗？" : "normal / fullscreen / scrollable / centered Modal"}</Modal.Body><Modal.Footer><Button variant="secondary" onClick={() => setModal(null)}>取消</Button><Button onClick={() => setModal(null)}>确定</Button></Modal.Footer></Modal><Offcanvas show={Boolean(drawer)} onHide={() => setDrawer(null)} placement={drawer ?? "end"}><Offcanvas.Header closeButton><Offcanvas.Title>Drawer / Sidebar</Offcanvas.Title></Offcanvas.Header><Offcanvas.Body>可从 {drawer} 方向打开的 Offcanvas。</Offcanvas.Body></Offcanvas><ToastContainer position="top-end" className="p-3"><Toast show={toast} onClose={() => setToast(false)} autohide delay={3000}><ToastHeader><strong className="me-auto">通知</strong><small>刚刚</small></ToastHeader><ToastBody><div className="d-flex justify-content-between align-items-center">Toast 四种背景变体与操作<Button size="sm" onClick={() => setToast(false)}>操作</Button></div></ToastBody></Toast></ToastContainer><Modal show={command} onHide={() => setCommand(false)} centered><Modal.Header closeButton><Modal.Title>CommandPalette</Modal.Title></Modal.Header><Modal.Body><Form.Control autoFocus placeholder="搜索命令..." className="mb-3" /><ListGroup><ListGroup.Item action>创建项目</ListGroup.Item><ListGroup.Item action>打开设置</ListGroup.Item></ListGroup></Modal.Body></Modal></ResponsivePage>
}
...
function CardGroupDemo() {
  return <CardGroup className="ms-2"><Card><Card.Body><CardImgOverlay>CardGroup / CardImgOverlay</CardImgOverlay></Card.Body></Card><Card><Card.Body><CardSubtitle>CardSubtitle</CardSubtitle><CardLink href="#">CardLink</CardLink></Card.Body></Card></CardGroup>
}
...
function ExportExtras() {
  return <Row className="g-2 mt-3"><Col md={4}><Figure><Figure.Image width={120} height={70} src="https://placehold.co/120x70" /><Figure.Caption>Figure caption</Figure.Caption></Figure></Col><Col md={4}><Form.Label htmlFor="export-select">FormSelect</Form.Label><FormSelect id="export-select"><option>Option</option></FormSelect></Col><Col md={4}><Fade in><div className="bg-body-secondary p-2">Fade export</div></Fade><Overlay show={false} target={document.body}><Popover>Overlay export</Popover></Overlay></Col></Row>
}
