...
<template>
  <DemoBlock v-for="name in names" :id="name" :key="name" :title="name">
    ...
    <template v-else-if="name === 'QSkeleton'">
      ...
      <q-skeleton height="90px" animation="pulse" class="q-mt-md" />
    </template>
    <template v-else-if="name === 'QSpinner'">
      <div class="row q-gutter-md items-center"><component :is="spinner" v-for="spinner in spinners" :key="spinner" :style="{ color: 'var(--q-primary)' }" size="24px" /></div>
    </template>
