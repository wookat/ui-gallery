...
function renderExtras(name: string, status: string) {
  ...
  if (name === "ColorChannelField") return <DemoCard name={name} status={status}><DemoFrame><div class="grid max-w-xs gap-2"><For each={["red", "green", "blue"] as const}>{(channel) => <ColorChannelField channel={channel} value={defaultColor} class="grid gap-1.5"><ColorChannelField.Label class="text-sm capitalize">{channel}</ColorChannelField.Label><ColorChannelField.Input class="h-9 rounded border border-zinc-300 px-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" /></ColorChannelField>}</For></div></DemoFrame></DemoCard>
