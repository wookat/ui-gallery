...
function SideNav({ expanded, onToggle, onNavigate }: { expanded: boolean; onToggle: () => void; onNavigate?: () => void }) {
  ...
  return <Sidenav expanded={expanded} appearance="default" activeKey={location.pathname} onSelect={() => onNavigate?.()}>
    <Sidenav.Header><div style={{ padding: 18, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden" }}><span className="avatar-letter" style={{ width: 28, height: 28, background: "var(--rs-primary-500)", color: "white", borderRadius: 7, marginRight: 8 }}>A</span>{expanded ? "Acme Console" : ""}</div></Sidenav.Header>
