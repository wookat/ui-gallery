...
<template>
  <div>
    <nav class="landing-nav"><RouterLink class="brand" to="/"><span class="brand-mark"><AppIcon name="zap" /></span>Acme Console</RouterLink><div class="desktop-only inline"><a v-for="link in links" :key="link" href="#features">{{ link }}</a><van-button type="primary" to="/login">开始使用</van-button></div><van-button class="mobile-only" plain @click="menu = true"><AppIcon name="menu" /></van-button></nav>
    ...
    <section class="hero"><div class="stack"><van-tag type="primary">ACME CONSOLE</van-tag><h1>{{ landing.hero.title }}</h1><p class="muted">{{ landing.hero.subtitle }}</p><div class="inline"><van-button type="primary" size="large" to="/login">{{ landing.hero.primary }}<AppIcon name="arrow-right" /></van-button><van-button plain size="large">{{ landing.hero.secondary }}</van-button></div><div class="inline muted"><span v-for="name in ['林', '王', 'A', 'M']" :key="name" class="initial">{{ name }}</span>{{ landing.hero.social }}</div></div><div class="hero-shot"><van-skeleton-image /><div class="screenshot-grid"><span /><span /><span /><span /></div></div></section>
    <section class="landing-section logo-cloud"><div v-for="name in ['Northwind', 'Contoso', 'Globex', 'Fabrikam', 'Initech', 'Umbrella']" :key="name" class="logo-placeholder">{{ name }}</div></section>
    <section id="features" class="landing-section"><div class="section-intro"><h2>一个地方，完成所有工作</h2><p class="muted">团队所需的工具</p></div><div class="grid grid-3"><div v-for="feature in landing.features" :key="feature.title" class="card"><AppIcon :name="feature.icon" :size="24" /><h3>{{ feature.title }}</h3><p class="muted">{{ feature.desc }}</p></div></div></section>
    ...
    <section id="pricing" class="landing-section"><div class="section-intro between"><div><h2>选择方案</h2><p class="muted">简单透明</p></div><div class="inline">月 <van-switch v-model="yearly" /> 年</div></div><div class="grid grid-3"><div v-for="plan in plans" :key="plan.name" class="card price-card"><div class="between"><h3>{{ plan.name }}</h3><van-tag v-if="plan.recommended" type="success">推荐</van-tag></div><strong>{{ plan.price === null ? "定制" : plan.price === 0 ? "免费" : `¥${yearly ? plan.price * 10 : plan.price}` }}</strong><van-cell v-for="item in plan.features" :key="item" :title="item"><template #icon><AppIcon name="check" :size="14" /></template></van-cell><van-button block type="primary" plain>开始使用</van-button></div></div></section>
    ...
    <footer class="landing-footer"><div><strong>Acme Console</strong><p class="muted">业务协同控制台</p></div><div v-for="column in ['产品', '资源', '公司']" :key="column" class="stack"><strong>{{ column }}</strong><a v-for="link in links.slice(0, 3)" :key="link" href="#features">{{ link }}</a></div><div class="inline"><AppIcon name="globe" /><AppIcon name="link" /><AppIcon name="star" /></div><small class="muted">© 2026 Acme Console</small></footer>
  </div>
</template>
