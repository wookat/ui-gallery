  ...
  <div class="landing">
    ...
    <main>
      ...
      <section class="logo-cloud">
        <span v-for="name in ['NORTHWIND', 'CONTOSO', 'FABRIKAM', 'GLOBEX', 'INITECH', 'UMBRELLA']" :key="name">{{ name }}</span>
      </section>
      ...
      <section class="section split" v-for="(feature, index) in landing.features.slice(0, 3)" :key="feature.title">
        ...
        <div>
          ...
          <h2>{{ feature.title }}</h2>
          <p>{{ feature.desc }} 通过清晰的工作流，团队可以快速从想法走向结果。</p>
      ...
      <section id="pricing" class="section">
        <div class="section-top">
          <div>
            <h2>简单透明的定价</h2>
            <p class="section-lead">按团队规模选择合适计划。</p>
          </div>
...
.hero-shot :deep(.el-skeleton),
.hero-shot :deep(.el-skeleton__item) {
  width: 100%;
  max-width: 100%;
  box-sizing: border-box;
}
