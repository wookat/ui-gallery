...
const sessionIcon = (device: string): IconName => device.includes("iPhone") ? "smartphone" : device.includes("MacBook") ? "laptop" : "monitor"
const invoiceStatusColor = (status: string) => status === "due" ? (theme.global.current.value.dark ? "warning" : "warning-darken-3") : "success"
const invoiceStatusLabel = (status: string) => status === "due" ? "待支付" : "已支付"
</script>
...
<template>
  <div>
    ...
    <template v-else>
      ...
      <v-tabs v-model="tab" show-arrows class="settings-tabs mb-6"><v-tab value="profile">个人资料</v-tab><v-tab value="security">账号安全</v-tab><v-tab value="notifications">通知</v-tab><v-tab value="team">团队</v-tab><v-tab value="billing">计费</v-tab></v-tabs>
      <v-window v-model="tab">
          ...
          <v-window-item value="billing"><v-card title="计费" subtitle="管理当前计划与发票"><v-card-text><v-card variant="tonal" color="primary" class="pa-4 mb-5"><div class="d-flex justify-space-between"><div><div class="text-overline">当前计划</div><div class="text-h5">Pro</div></div><v-chip>每月 ¥99</v-chip></div></v-card><v-row><v-col v-for="plan in plans" :key="plan.name" cols="12" sm="4"><v-card height="100%" variant="outlined" class="pa-4"><div class="d-flex justify-space-between align-start"><span class="text-h6">{{ plan.name }}</span><v-chip v-if="plan.recommended" size="small" color="primary">推荐</v-chip></div><div class="text-h5 my-3">{{ plan.price === null ? "联系我们" : `¥${plan.price}/月` }}</div><v-list density="compact"><v-list-item v-for="feature in plan.features" :key="feature" :title="feature"><template #prepend><Icon name="check" size="16" /></template></v-list-item></v-list></v-card></v-col></v-row><div class="text-subtitle-1 mt-6 mb-2">发票记录</div><v-data-table :headers="[{ title: '编号', key: 'id' }, { title: '日期', key: 'date' }, { title: '金额', key: 'amount' }, { title: '状态', key: 'status' }]" :items="invoices" density="comfortable"><template #[`item.status`]="{ item }"><v-chip size="small" :color="invoiceStatusColor(item.status)" variant="tonal">{{ invoiceStatusLabel(item.status) }}</v-chip></template></v-data-table></v-card-text></v-card></v-window-item>
        </v-window>
      <v-card variant="outlined" class="mt-6 danger-card"><template #title><span class="text-error">危险区</span></template><template #subtitle>删除账号后所有数据将无法恢复</template><template #append><v-btn color="error" variant="outlined" @click="removeDialog = true">删除账号</v-btn></template></v-card>
...
<style scoped>
.danger-card { border-color: rgb(var(--v-theme-error)); }
</style>
