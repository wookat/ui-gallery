...
function CodeBlock({ children }: { children?: ReactNode }) {
  ...
  const copy = () => { void navigator.clipboard?.writeText(ref.current?.textContent ?? ""); setCopied(true); window.setTimeout(() => setCopied(false), 1500) }
  return <div className="relative chat-code"><pre ref={ref}>{children}</pre><Button type="button" text size="small" className="absolute top-0 right-0 chat-copy" icon={<Icon name={copied ? "check" : "copy"} />} aria-label="复制代码" tooltip={copied ? "已复制" : "复制代码"} tooltipOptions={{ position: "left" }} onClick={copy} /></div>
}
