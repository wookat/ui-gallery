  ...
  return <div class="space-y-6">
    ...
    <Tabs.Root value={tab()} onChange={setTab} orientation="vertical" class="grid gap-6 md:grid-cols-[180px_minmax(0,1fr)]">
      <Tabs.List class="flex gap-1 overflow-x-auto border-b border-zinc-200 pb-2 md:grid md:h-fit md:border-b-0 md:border-r md:pb-0 md:pr-3 dark:border-zinc-800">
        <For each={[["profile", "个人资料"], ["security", "账号安全"], ["notifications", "通知"], ["team", "团队"], ["billing", "计费"]]}>{(item) => <Tabs.Trigger value={item[0]} class="whitespace-nowrap rounded-md px-3 py-2 text-left text-sm data-[selected]:bg-zinc-100 data-[selected]:font-medium dark:data-[selected]:bg-zinc-800">{item[1]}</Tabs.Trigger>}</For>
      </Tabs.List>
      <div class="min-w-0">
    ...
    <Dialog.Root open={deleteOpen()} onOpenChange={setDeleteOpen}><Dialog.Portal><Dialog.Overlay class="fixed inset-0 z-50 bg-black/40" /><Dialog.Content class="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900"><Dialog.Title class="text-lg font-semibold">确认删除账号</Dialog.Title><Dialog.Description class="mt-2 text-sm text-zinc-500">请输入 DELETE 以确认此操作。</Dialog.Description><TextField class="mt-4" label="确认文字" value={confirmText()} onInput={(event) => setConfirmText(event.currentTarget.value)} /><div class="mt-5 flex justify-end gap-2"><Dialog.CloseButton class="rounded-md border px-3 py-2 text-sm">取消</Dialog.CloseButton><Button variant="destructive" disabled={confirmText() !== "DELETE"}>永久删除</Button></div></Dialog.Content></Dialog.Portal></Dialog.Root>
  </div>
}
