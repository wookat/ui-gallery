...
const current = ref(0)
const form = ref()
const model = ref({ name: "", count: 1, email: "", phone: "", bio: "", type: "team", options: ["通知"], enabled: true, plan: "pro", tags: [], color: "#1677ff" })
const planOptions = [{ value: "starter", label: "Starter" }, { value: "pro", label: "Pro" }, { value: "enterprise", label: "Enterprise" }]
const planLabel = computed(() => planOptions.find((item) => item.value === model.value.plan)?.label ?? model.value.plan)
  ...
  <div class="page">
    ...
    <a-card class="section">
      ...
      <a-form v-else ref="form" :model="model" :rules="rules" layout="vertical">
        ...
        <template v-else-if="current === 1"><div class="form-grid"><a-form-item label="计划"><a-select v-model:value="model.plan" :options="planOptions" /></a-form-item><a-form-item label="标签"><a-select v-model:value="model.tags" mode="tags" /></a-form-item><a-form-item label="自动完成"><a-auto-complete placeholder="输入关键字" /></a-form-item><a-form-item label="日期"><a-date-picker style="width:100%" /></a-form-item><a-form-item label="时间"><a-time-picker style="width:100%" /></a-form-item><a-form-item label="日期范围"><a-range-picker /></a-form-item><a-form-item label="优先级"><a-slider range :default-value="[20, 80]" /></a-form-item><a-form-item label="评分"><a-rate /></a-form-item><a-form-item label="颜色（composed）"><a-input v-model:value="model.color" placeholder="#1677ff"><template #prefix><input v-model="model.color" type="color" class="color-swatch" aria-label="选择颜色"></template></a-input></a-form-item></div><a-form-item label="上传文件"><a-upload-dragger><p>拖拽文件到这里，或点击上传</p><p class="muted">支持常见文档格式</p></a-upload-dragger></a-form-item></template>
        <template v-else><a-descriptions bordered :column="1"><a-descriptions-item label="项目名称">{{ model.name || "未填写" }}</a-descriptions-item><a-descriptions-item label="负责人邮箱">{{ model.email || "未填写" }}</a-descriptions-item><a-descriptions-item label="计划">{{ planLabel }}</a-descriptions-item><a-descriptions-item label="自动化">{{ model.enabled ? "已启用" : "已关闭" }}</a-descriptions-item></a-descriptions><a-checkbox class="section">我已确认以上信息</a-checkbox></template>
...
<style scoped>.color-swatch { width:24px; height:24px; padding:0; border:0; background:none; cursor:pointer; }.color-swatch::-webkit-color-swatch-wrapper { padding:0; }.color-swatch::-webkit-color-swatch { border:1px solid var(--app-color-border); border-radius:4px; }.form-grid { display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0 18px; }.form-actions { display:flex; justify-content:flex-end; gap: 10px; margin-top: 24px; } @media (max-width: 600px) { .form-grid { grid-template-columns: 1fr; } }</style>
