...
import { Alert, Button, Card, Checkbox, Description, Form, Input, InputGroup, Label, Link, TextField } from "@heroui/react"
    ...
    <div className="grid min-h-svh bg-background text-foreground lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-6">
        <Card className="w-full max-w-sm">
          <Card.Header><Card.Title className="text-2xl">登录</Card.Title><Card.Description>输入邮箱与密码进入控制台。</Card.Description></Card.Header>
          <Card.Content>
            <Form onSubmit={submit} className="space-y-4">
              ...
              <TextField name="email" type="email" isRequired defaultValue="m0@acme.dev">
                <Label>邮箱</Label>
                <Input placeholder="you@acme.dev" />
              </TextField>
              <TextField name="password" type={showPassword ? "text" : "password"} isRequired>
                ...
                <InputGroup>
                  ...
                  <InputGroup.Suffix><button type="button" aria-label={showPassword ? "隐藏密码" : "显示密码"} className="text-muted" onClick={() => setShowPassword((v) => !v)}><Icon name={showPassword ? "eye-off" : "eye"} size={16} /></button></InputGroup.Suffix>
                </InputGroup>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Button variant="secondary" onPress={() => navigate("/")}><Icon name="mail" size={16} />邮箱链接</Button>
              </div>
            </Form>
          </Card.Content>
