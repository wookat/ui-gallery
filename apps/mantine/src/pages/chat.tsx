                ...
                return (
                  <Group key={i} align="flex-start" wrap="nowrap" justify={user ? "flex-end" : "flex-start"}>
                    ...
                    <Stack gap={6} maw="85%" align={user ? "flex-end" : "flex-start"} style={{ minWidth: 0 }}>
                      ...
                      {user ? null : <Group gap={2}><CopyButton value={m.content}>{({ copied, copy }) => <Tooltip label={copied ? "已复制" : "复制"}><ActionIcon size="lg" variant="subtle" color="gray" onClick={copy} aria-label="复制"><Icon name={copied ? "check" : "copy"} size={15} /></ActionIcon></Tooltip>}</CopyButton><ActionIcon size="lg" variant="subtle" color="gray" aria-label="赞"><Icon name="heart" size={15} /></ActionIcon><ActionIcon size="lg" variant="subtle" color="gray" aria-label="踩"><Icon name="arrow-down" size={15} /></ActionIcon><ActionIcon size="lg" variant="subtle" color="gray" aria-label="重试"><Icon name="refresh" size={15} /></ActionIcon></Group>}
                    </Stack>
