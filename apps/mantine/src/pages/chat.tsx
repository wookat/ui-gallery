...
function ConversationList({ onPick }: { onPick?: () => void }) {
  ...
  return (
    <Stack gap="sm" h="100%">
      ...
      <Text size="xs" fw={600} c="dimmed" tt="uppercase">最近</Text>
      <Stack gap={2}>
        {items.map((c, i) => (
      ...
      <Paper withBorder radius="md" style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        <Group justify="space-between" p="sm" wrap="nowrap" style={{ borderBottom: "1px solid var(--mantine-color-default-border)" }}>
          <Group gap="xs" wrap="nowrap" style={{ minWidth: 0 }}>
            <ActionIcon variant="subtle" color="gray" hiddenFrom="md" onClick={() => setDrawer(true)} aria-label="对话列表"><Icon name="menu" size={18} /></ActionIcon>
            <div style={{ minWidth: 0 }}><Text fw={600} size="sm" truncate>{chat.conversations[0].title}</Text><Text size="xs" c="dimmed">{model} · 已连接</Text></div>
          </Group>
                ...
                return (
                  <Group key={i} align="flex-start" wrap="nowrap" justify={user ? "flex-end" : "flex-start"}>
                    ...
                    <Stack gap={6} maw="85%" align={user ? "flex-end" : "flex-start"} style={{ minWidth: 0 }}>
                      <Group gap="xs"><Text size="xs" fw={500}>{user ? "林晓" : "AI 助手"}</Text><Text size="xs" c="dimmed">刚刚</Text></Group>
                      {m.tool ? (
                      ...
                      <Paper radius="md" px="md" py="sm" bg={user ? "var(--mantine-primary-color-filled)" : "var(--mantine-color-default)"} c={user ? "white" : undefined} style={{ maxWidth: "100%", overflow: "hidden" }}>
                        ...
                        {m.streaming ? <Group gap={6} mt={4}><Loader size={12} type="dots" /><Text size="xs" c="dimmed">正在生成...</Text></Group> : null}
                      </Paper>
                      ...
                      {user ? null : <Group gap={2}><CopyButton value={m.content}>{({ copied, copy }) => <Tooltip label={copied ? "已复制" : "复制"}><ActionIcon size="sm" variant="subtle" color="gray" onClick={copy} aria-label="复制"><Icon name={copied ? "check" : "copy"} size={13} /></ActionIcon></Tooltip>}</CopyButton><ActionIcon size="sm" variant="subtle" color="gray" aria-label="赞"><Icon name="heart" size={13} /></ActionIcon><ActionIcon size="sm" variant="subtle" color="gray" aria-label="踩"><Icon name="arrow-down" size={13} /></ActionIcon><ActionIcon size="sm" variant="subtle" color="gray" aria-label="重试"><Icon name="refresh" size={13} /></ActionIcon></Group>}
                    </Stack>
        ...
        <Box p="sm" style={{ borderTop: "1px solid var(--mantine-color-default-border)" }}>
          ...
          <Paper withBorder radius="md" p="xs">
            ...
            <Group justify="space-between" mt={4} wrap="nowrap">
              <Group gap={4}><ActionIcon variant="subtle" color="gray" aria-label="附件"><Icon name="paperclip" size={16} /></ActionIcon><Text size="xs" c="dimmed" visibleFrom="sm">{draft.length}/2000 · <Kbd size="xs">Enter</Kbd> 发送，<Kbd size="xs">Shift</Kbd>+<Kbd size="xs">Enter</Kbd> 换行</Text></Group>
