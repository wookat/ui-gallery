...
- 组件先查 `04-components.md`：有就用；没有先从 shadcn 注册表拷入 `src/components/ui/`，删掉默认类、只用主题类、加进 `/kitchen-sink` 状态矩阵，再在映射表补一行。业务组合件放 `src/components/composed/`。第 2 轮（orders / form / settings / components / landing / chat）所需控件已就位：`Textarea` `NumberInput` `Select`（原生 select）`Combobox` `RadioGroup` `Switch` `Slider` `Calendar`/`DatePicker` `TagInput` `OTPInput` `FileDropzone` `Dialog` `AlertDialog` `Drawer` `Accordion` `Pagination` `Segmented` `DescriptionList`；组合件 `Stepper` `PageHeader` `Toolbar` `Result` `PricingCard` `CodeBlock` `ChatBubble` `ToolCall` `SourceChip` `SuggestionChip` `Composer` `ConversationItem`。屏幕实现只组合这些件，不在页面里重写控件外观。
- 组件不写任何字面文案：可访问名 / 按钮文案一律由调用方通过 props 传入（`labels` / `closeLabel` / `removeLabel` / `statusLabels` / `cellLabel` …），页面层用 `t(key)` 取值。浮层（Dialog / AlertDialog / Drawer / Combobox / DatePicker / Popover）都支持受控 `open` / `onOpenChange`，屏幕里绑到 `useScreenState()` 的 `?open=<id>`，并在 `shots.json` 加 `overlay: true` 条目；Radix Popover 内容必须有 `aria-label`。
- 下拉选择用原生 `<select>`（`Select`），不自绘面板（与 hifi 一致，且 Radix Select 的 aria-hidden 焦点圈定会被 axe `aria-hidden-focus` 判为 serious）；需要搜索 / 副文案的选项用 `Combobox`。可换行的小件（`SourceChip` / `Chip`）容器用 `gap-y-3`，否则 `hit-area` 的伪元素会盖住上一行热区。
