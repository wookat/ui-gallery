...
for (const [vp, [width, height]] of Object.entries(viewports)) {
  for (const theme of themes) {
    ...
    for (const [state, query] of Object.entries(states)) {
      ...
      shots++;
      const { sw, bodySw, smallHits } = await page.evaluate((hit) => {
        ...
        return { sw: document.documentElement.scrollWidth, bodySw: document.body.scrollWidth, smallHits };
      }, HIT);
      const bytes = statSync(file).size;
      const ok = sw <= width && bodySw <= width && errors.length === 0 && smallHits.length === 0 && bytes < MAX_BYTES;
      if (!ok) fail++;
      console.log(
        `${ok ? "ok  " : "FAIL"} ${name.padEnd(34)} scrollWidth=${sw}/${bodySw} (vp ${width}) errors=${errors.length} smallHits=${smallHits.length} size=${(bytes / 1024).toFixed(0)}KB`,
      );
