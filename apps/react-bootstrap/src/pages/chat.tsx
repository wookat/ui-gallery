...
export function ChatPage() {
  ...
  const activeConversation = chat.conversations[0]
  return <ResponsivePage><PageTitle title="AI 助手" subtitle="用自然语言探索你的业务数据。" actions={<div className="d-flex gap-2"><Button variant="outline-secondary" onClick={() => setEmpty(!empty)}>{empty ? "显示消息流" : "查看空态"}</Button><Button className="d-lg-none" onClick={() => setMobile(true)}><Icon name="list" className="me-1" />会话列表</Button></div>} /><Row className="chat-layout border rounded overflow-hidden g-0"><Col lg={3} className="d-none d-lg-flex flex-column border-end p-3"><ConversationList query={query} setQuery={setQuery} conversations={conversations} /></Col><Col className="d-flex flex-column min-width-0"><div className="chat-stream p-3 p-md-4 flex-grow-1">{empty || chat.messages.length === 0 ? <div className="py-4"><EmptyState title="开始一段新对话" /><Row className="g-2">{chat.suggestions.map((suggestion) => <Col sm={6} key={suggestion}><Card className="h-100"><Card.Body><Button variant="link" className="p-0 text-start" onClick={() => setText(suggestion)}>{suggestion}</Button></Card.Body></Card></Col>)}</Row></div> : chat.messages.map((message, index) => <div key={`${message.role}-${index}`} className={`d-flex gap-2 mb-3 ${message.role === "user" ? "justify-content-end" : ""}`}><Avatar name={message.role === "user" ? "林" : "AI"} /><Card className={message.role === "user" ? "bg-primary text-white" : ""} style={{ maxWidth: "min(720px, 88%)" }}><Card.Body className="p-3"><ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>{message.content}</ReactMarkdown>{message.sources && <div className="d-flex gap-1 flex-wrap mt-2">{message.sources.map((source) => <Badge bg="secondary" pill key={source}>{source}</Badge>)}</div>}{message.tool && <><Button variant="link" size="sm" onClick={() => setCollapsed(!collapsed)}>工具调用：{message.tool.name}</Button><Collapse in={!collapsed}><div className="small bg-body-secondary rounded p-2">{JSON.stringify(message.tool.args)}</div></Collapse></>}{message.streaming && <div className="text-body-secondary small"><Spinner animation="grow" size="sm" /> 正在输入</div>}<small className={`d-block mt-2 ${message.role === "user" ? "text-white" : "text-body-secondary"}`}>{activeConversation.time}</small></Card.Body></Card></div>)}</div><div className="border-top p-3"><InputGroup><Form.Control as="textarea" rows={2} value={text} onChange={(event) => setText(event.target.value)} placeholder="输入消息... Enter 发送，Shift+Enter 换行" /><Button variant="outline-secondary" aria-label="附件"><Icon name="paperclip" /></Button><Button aria-label="发送"><Icon name="send" /></Button></InputGroup><div className="d-flex flex-wrap gap-2 mt-2">{chat.suggestions.map((suggestion) => <Button variant="outline-secondary" key={suggestion} onClick={() => setText(suggestion)}>{suggestion}</Button>)}</div><div className="d-flex justify-content-between"><small className="text-body-secondary mt-1">{text.length} / 4,000</small><small className="text-body-secondary mt-1">Enter 发送 / Shift+Enter 换行</small></div><Form.Select className="mt-2" aria-label="模型"><option value={chat.models[0]}>{chat.models[0]}</option>{chat.models.slice(1).map((model) => <option key={model}>{model}</option>)}</Form.Select></div></Col></Row><Offcanvas show={mobile} onHide={() => setMobile(false)}><Offcanvas.Header closeButton><Offcanvas.Title>会话列表</Offcanvas.Title></Offcanvas.Header><Offcanvas.Body><ConversationList query={query} setQuery={setQuery} conversations={conversations} /></Offcanvas.Body></Offcanvas></ResponsivePage>
}
