...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <Container>
    ...
    <Container>
      <Header><Navbar appearance="default"><Navbar.Brand as={Link} to="/" className="mobile-only">ACME</Navbar.Brand><Nav style={{ flex: 1 }}>{isMobile ? <IconButton className="mobile-only" size="lg" aria-label="打开导航" icon={<Icon name="menu" />} onClick={() => setDrawer(true)} /> : null}<Breadcrumb className="desktop-only"><Breadcrumb.Item as={Link} to="/">首页</Breadcrumb.Item><Breadcrumb.Item active>{current}</Breadcrumb.Item></Breadcrumb></Nav><Nav pullRight><InputGroup inside className="desktop-only" style={{ width: 210, margin: "8px 12px" }}><InputGroup.Addon><Icon name="search" size={14} /></InputGroup.Addon><Input placeholder="搜索" /></InputGroup><Whisper trigger="click" placement="bottomEnd" speaker={<NotificationPopover />}><IconButton size="lg" aria-label="通知" icon={<Badge content={notifications.filter((item) => item.unread).length}><Icon name="bell" /></Badge>} /></Whisper><IconButton size="lg" aria-label="切换主题" icon={<Icon name={theme === "dark" ? "sun" : "moon"} />} onClick={toggleTheme} /><Dropdown placement="bottomEnd" renderToggle={(props, ref) => <IconButton {...props} ref={ref} size="lg" aria-label="账户菜单" icon={<Avatar circle size="sm">林</Avatar>} />}><Dropdown.Item>个人资料</Dropdown.Item><Dropdown.Item>账号设置</Dropdown.Item><Dropdown.Item>帮助中心</Dropdown.Item><Dropdown.Item divider /><Dropdown.Item>退出登录</Dropdown.Item></Dropdown></Nav></Navbar></Header>
