...
<template>
  <div class="page">
    ...
    <van-form v-else ref="formRef" @submit="submit">
      <div v-if="step === 0" class="card form-card"><van-cell-group inset><van-field v-model="name" label="项目名称" required placeholder="项目名称" :rules="[{ required: true, message: '请输入项目名称' }]" /><van-field v-model="email" label="邮箱" type="email" required placeholder="邮箱" :rules="[{ required: true, message: '请输入邮箱' }, { pattern: /.+@.+/, message: '邮箱格式不正确' }]" /><van-field v-model="phone" label="电话" type="tel" required placeholder="电话" :rules="[{ required: true, message: '请输入电话' }]"><template #left-icon><van-button size="mini" plain @click="countryPopup = true">{{ country }}</van-button></template></van-field><van-field v-model="description" label="描述" type="textarea" maxlength="120" show-word-limit placeholder="项目描述" /><van-field label="人数" type="digit" placeholder="请输入人数" /><van-radio-group v-model="plan"><van-cell title="Pro" clickable @click="plan = 'Pro'"><template #right-icon><van-radio name="Pro" /></template></van-cell><van-cell title="Team" clickable @click="plan = 'Team'"><template #right-icon><van-radio name="Team" /></template></van-cell></van-radio-group><van-checkbox-group v-model="selectedOptions"><van-cell title="邮件通知"><template #right-icon><van-checkbox name="email" /></template></van-cell><van-cell title="短信通知"><template #right-icon><van-checkbox name="sms" /></template></van-cell></van-checkbox-group><van-cell title="接收通知"><template #right-icon><van-switch v-model="notifications" /></template></van-cell></van-cell-group><div class="form-actions"><van-button type="primary" @click="nextStep(1)">下一步</van-button></div></div>
