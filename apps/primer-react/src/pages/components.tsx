...
function TableDemo({ dataGrid = false }: { dataGrid?: boolean }) {
  const rows: OrderRow[] = orders.slice(0, 4).map((order) => ({
    id: order.id,
  ...
  if (dataGrid) {
    return (
      <div className="table-scroll stack-3">
        <DataTable data={rows} columns={columns} initialSortColumn="id" initialSortDirection="ASC" />
        <Pagination pageCount={3} currentPage={1} onPageChange={() => undefined} />
      </div>
    )
  }
  return (
    <div className="table-scroll stack-3">
      {(["condensed", "normal", "spacious"] as const).map((cellPadding) => (
        <Table key={cellPadding} cellPadding={cellPadding} gridTemplateColumns="minmax(max-content, 1fr) minmax(120px, 1fr) auto">
          <Table.Title id={`table-title-${cellPadding}`}>订单表 · {cellPadding}</Table.Title>
...
function Demo({ name }: { name: string }) {
  ...
  if (name === "Checkbox") return <CheckboxGroup aria-label="复选框状态" aria-labelledby="checkbox-label"><FormControl><FormControl.Label id="checkbox-label">复选框状态</FormControl.Label><label><Checkbox defaultChecked />已选中</label><label><Checkbox indeterminate />不确定</label><label><Checkbox disabled />已禁用</label></FormControl></CheckboxGroup>
  if (name === "Radio") return <RadioGroup name="component-radio" aria-label="单选框状态" aria-labelledby="radio-label"><span id="radio-label" hidden>单选框状态</span><label><Radio value="one" defaultChecked />选项一</label><label><Radio value="two" />选项二</label><label><Radio value="disabled" disabled />禁用选项</label></RadioGroup>
  ...
  if (name === "Rating") return <Stack direction="horizontal" gap="condensed">{Array.from({ length: 5 }, (_, index) => <IconButton key={index} aria-label={`${index + 1} 星`} variant="invisible" icon={iconFor(index < 4 ? "star-fill" : "star")} />)}</Stack>
  ...
  if (name === "Avatar") return <Stack direction="horizontal" gap="condensed" align="center">{[16, 24, 32, 48].map((size) => <Avatar key={size} src={avatarSrc} alt={`${size} avatar`} size={size} square />)}</Stack>
  if (name === "AvatarGroup") return <AvatarStack alignRight variant="cascade" size={32}>{["林晓", "王敏", "赵宁"].map((person) => <Avatar key={person} src={avatarSrc} alt={person} />)}</AvatarStack>
  ...
  if (name === "Image") return <figure><Avatar src={avatarSrc} alt="方形图像占位" size={96} square /><figcaption className="muted">Avatar square image</figcaption></figure>
  ...
  if (name === "Anchor") return <ActionList>{(contract.components as string[]).slice(0, 4).map((item) => <ActionList.LinkItem key={item} href={`#component-${item}`}>{item}</ActionList.LinkItem>)}</ActionList>
  if (name === "BackTop") return <IconButton aria-label="返回顶部" icon={iconFor("arrow-up")} onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} />
  ...
  if (name === "ScrollArea") return <ScrollableRegion style={{ maxHeight: 90 }} aria-label="可滚动内容"><div style={{ minHeight: 220 }}>ScrollableRegion 内容<br />向下滚动查看更多内容</div></ScrollableRegion>
  if (name === "Accordion") return <Details><Details.Summary>Accordion 项目</Details.Summary><Text as="p">Details disclosure 内容。</Text></Details>
  if (name === "ThemeProvider") return <Button onClick={() => setMode(mode === "dark" ? "light" : "dark")}>切换主题（当前 {mode}）</Button>
  if (name === "FloatButton") return <IconButton className="float-button" aria-label="创建项目" variant="primary" size="large" icon={iconFor("plus")} />
...
export function ComponentsPage() {
  ...
  return (
    <div className="page-stack">
      ...
      <div id="component-index" className="flex wrap gap-2">{components.map((name) => <a href={`#component-${name}`} key={name}>{name}</a>)}</div>
      <div className="grid grid-3">{components.map((name) => <section className="card" id={`component-${name}`} key={name}><div className="flex items-center justify-between gap-2"><Heading as="h2" className="card-title">{name}</Heading><Label variant={coverage[name] === "missing" ? "danger" : coverage[name] === "composed" ? "attention" : "success"}>{coverage[name]}</Label></div><Text as="p" className="muted">default · variants · sizes · states</Text><div style={{ marginTop: 16 }}><Demo name={name} /></div></section>)}</div>
    </div>
  )
}
