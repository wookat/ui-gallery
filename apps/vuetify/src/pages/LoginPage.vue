  ...
  <v-main class="d-flex align-center justify-center pa-4 login-page">
    <v-card class="w-100 pa-4 pa-sm-8" max-width="420" elevation="4">
      ...
      <v-form ref="form" @submit.prevent="submit">
        ...
        >
          <template #append-inner>
            <v-btn icon variant="text" :aria-label="showPassword ? '隐藏密码' : '显示密码'" @click="showPassword = !showPassword">
              <Icon :name="showPassword ? 'eye-off' : 'eye'" size="18" />
            </v-btn>
          </template>
        </v-text-field>
        <div class="d-flex align-center justify-space-between flex-wrap ga-2 mb-4">
          <v-checkbox v-model="remember" label="记住我" hide-details />
          <v-btn variant="text" size="large">忘记密码？</v-btn>
        </div>
      ...
      <div class="d-flex flex-column ga-2">
        <v-btn variant="outlined" block size="large"><template #prepend><Icon name="google" /></template>使用 Google 登录</v-btn>
        <v-btn variant="outlined" block size="large"><template #prepend><Icon name="github" /></template>使用 GitHub 登录</v-btn>
        <v-btn variant="outlined" block size="large"><template #prepend><Icon name="wechat" /></template>使用微信登录</v-btn>
      </div>
      <p class="text-center text-body-2 text-medium-emphasis mt-6 mb-0">没有账号？<v-btn variant="text" size="large" class="px-1">注册</v-btn></p>
    </v-card>
  </v-main>
</template>
