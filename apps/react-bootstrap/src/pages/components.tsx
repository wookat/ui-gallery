...
function ComponentDemo({ name, openModal, openDrawer, openToast, openCommand, toasts, closeToast, resetToasts }: { name: string; openModal: (mode: string) => void; openDrawer: (placement: "start" | "end" | "top" | "bottom") => void; openToast: () => void; openCommand: () => void; toasts: string[]; closeToast: (variant: string) => void; resetToasts: () => void }) {
  ...
  switch (name) {
    ...
    case "Radio": return <div className="d-flex flex-wrap align-items-center gap-3"><Form.Check type="radio" name="radio-demo" label="内联" inline defaultChecked className="m-0" /><Form.Check type="radio" name="radio-demo" label="另一个" inline className="m-0" /><Form.Check type="radio" name="radio-demo-reverse" label="反向" reverse className="reverse-inline m-0" /></div>
    ...
    case "Badge": return <div className="d-flex flex-wrap gap-2">{["primary", "secondary", "success", "danger", "warning", "info", "light", "dark"].map((variant) => <Badge key={variant} bg={variant} text={variant === "light" || variant === "warning" ? "dark" : undefined} className={variant === "dark" ? "border border-secondary" : undefined}>{variant}</Badge>)}<Badge pill bg="primary">pill</Badge><Button className="position-relative">消息<Badge pill bg="danger" className="position-absolute top-0 start-100 translate-middle">4</Badge></Button></div>
    case "Tag": return <div className="d-flex flex-wrap gap-2">{["标签", "React Bootstrap", "可关闭"].map((label) => <Badge key={label} pill bg="primary" className="tag-pill d-inline-flex align-items-center gap-1 text-nowrap">{label}<CloseButton variant="white" aria-label={`移除 ${label}`} /></Badge>)}</div>
    ...
    case "Carousel": return <Carousel indicators data-bs-theme="light" className="rounded overflow-hidden"><CarouselItem><div className="text-bg-dark" style={{ height: 200 }} /><Carousel.Caption><h3 className="h5">第一张</h3><p className="mb-0">带 captions 的 Carousel</p></Carousel.Caption></CarouselItem><CarouselItem><div className="text-bg-secondary" style={{ height: 200 }} /><Carousel.Caption><h3 className="h5">第二张</h3><p className="mb-0">第二张幻灯片</p></Carousel.Caption></CarouselItem></Carousel>
...
function TabsDemo() {
  const tabs = [["one", "标签一", "内容一"], ["two", "标签二", "内容二"], ["three", "标签三", "内容三"]] as const
  const render = (variant: "tabs" | "pills" | "underline", label: string, extra?: { fill?: boolean; justify?: boolean }) => <div key={label}><div className="small text-body-secondary mb-1">{label}</div><Tabs defaultActiveKey="one" variant={variant} fill={extra?.fill} justify={extra?.justify} className="mb-2">{tabs.map(([key, title, body]) => <Tab key={key} eventKey={key} title={title}>{body}</Tab>)}</Tabs></div>
  return <div className="d-grid gap-3">{render("tabs", "tabs（默认）")}{render("pills", "pills")}{render("underline", "underline")}{render("tabs", "fill", { fill: true })}{render("pills", "justified", { justify: true })}</div>
}

function ToastLevelsDemo({ shown, onClose, onReset }: { shown: string[]; onClose: (variant: string) => void; onReset: () => void }) {
  return <div className="d-grid gap-2">{["primary", "success", "warning", "danger"].map((variant) => <Toast key={variant} bg={variant} show={shown.includes(variant)} onClose={() => onClose(variant)} className={variant === "warning" ? "text-dark" : "text-white"} style={{ width: "100%", maxWidth: "100%" }}><Toast.Header><strong className="me-auto">{variant} 通知</strong><small>示例</small></Toast.Header><Toast.Body className="d-flex justify-content-between align-items-center"><span>{variant} 级别 Toast</span><Button size="sm" variant={variant === "warning" ? "dark" : "light"} onClick={() => onClose(variant)}>操作</Button></Toast.Body></Toast>)}<Button variant="outline-secondary" onClick={onReset}>重新显示全部</Button></div>
}
function CheckboxDemo() {
  return <div className="d-flex flex-wrap align-items-center gap-3"><Form.Check id="cb-inline" label="Inline" inline defaultChecked className="m-0" /><Form.Check id="cb-reverse" label="Reverse" reverse className="reverse-inline m-0" /><Form.Check id="cb-disabled" label="Disabled" disabled className="m-0" /><Form.Check id="cb-indeterminate" label="Indeterminate style" className="m-0" /></div>
}
