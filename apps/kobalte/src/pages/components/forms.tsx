...
function renderForms(name: string, status: string) {
  ...
  if (name === "DatePicker" || name === "DateRangePicker") return <DemoCard name={name} status={status}><DemoFrame><div class="flex flex-wrap gap-3"><Show when={name === "DateRangePicker"} fallback={<TextField label="开始日期" type="date" value="2026-09-01" />}><DateRangePicker label="日期范围（Popover 组合）" class="w-72" value={{ start: "2026-09-01", end: "2026-09-30" }} /></Show></div><div class="mt-4 grid max-w-sm grid-cols-7 gap-1 text-center text-xs"><For each={["一","二","三","四","五","六","日"]}>{(day) => <span class="font-medium text-zinc-500 dark:text-zinc-400">{day}</span>}</For>{Array.from({ length: 30 }, (_, i) => <button class={`rounded p-1 ${i === 14 ? "bg-blue-600 text-white" : "hover:bg-zinc-200 dark:hover:bg-zinc-800"}`}>{i + 1}</button>)}</div></DemoFrame></DemoCard>
  ...
  if (name === "Checkbox") return <DemoCard name={name} status={status}><DemoFrame><div class="flex flex-wrap gap-4"><Checkbox label="未选中" /><Checkbox label="已选中" checked /><Checkbox label="不确定" indeterminate /><Checkbox label="禁用" disabled /><Checkbox label="错误" class="text-red-600 dark:text-red-400" /></div></DemoFrame></DemoCard>
  ...
  if (name === "Select") return <DemoCard name={name} status={status}><DemoFrame><div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Select label="默认（占位）" options={options} placeholder="请选择一项" /><Select label="已选值" options={options} value="b" /><Select label="禁用" options={options} value="a" disabled /><div><Select label="错误" options={options} placeholder="必填项" class="border-red-500 dark:border-red-500" /><p class="mt-1 text-xs text-red-600 dark:text-red-400">请选择一个选项</p></div></div></DemoFrame></DemoCard>
