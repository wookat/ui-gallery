...
import { checkboxHit, switchHit } from '@/pages/shared'
  ...
  <div class="space-y-6">
    ...
    <template v-if="!submitted">
      ...
      <div v-if="step === 'basic'"><Card><CardHeader><CardTitle>基本信息</CardTitle><CardDescription>告诉我们项目的基础信息。</CardDescription></CardHeader><CardContent class="space-y-6"><FieldSet class="grid gap-5 sm:grid-cols-2">
        ...
        <Field class="sm:col-span-2"><FieldLabel>通知渠道</FieldLabel><FieldContent><div class="flex flex-wrap gap-4"><label v-for="channel in channelOptions" :key="channel" class="flex min-h-10 items-center gap-2 text-sm"><Checkbox :class="checkboxHit" :model-value="notificationChannels.includes(channel)" @update:model-value="toggleChannel(channel, $event)" />{{ channel }}</label></div></FieldContent></Field>
        ...
        <div class="flex items-center justify-between rounded-lg border p-4 sm:col-span-2"><div><p class="font-medium">接收项目通知</p><p class="text-sm text-muted-foreground">通过邮件接收项目更新。</p></div><Switch v-model="receiveNotifications" :class="switchHit" /></div>
