...
const filtered = computed(() => orders.filter((item) => {
...
<template>
  <div class="space-y-6">
    ...
    <Card>
      <CardHeader class="gap-4"><div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><CardTitle>订单列表</CardTitle><Button variant="outline" class="min-h-10" @click="toast.success('导出任务已创建')"><Icon name="download" />导出</Button></div></CardHeader>
      <CardContent class="space-y-4">
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_160px_180px_auto_auto]">
          ...
          <DropdownMenu><DropdownMenuTrigger as-child><Button variant="outline" class="min-h-10"><Icon name="filter" />渠道</Button></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuLabel>渠道筛选</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuCheckboxItem v-for="channel in ['web', 'ios', 'android', 'api']" :key="channel" :checked="channels.includes(channel)" @update:checked="toggleChannel(channel, $event)">{{ channel }}</DropdownMenuCheckboxItem></DropdownMenuContent></DropdownMenu>
          <DropdownMenu><DropdownMenuTrigger as-child><Button variant="outline" class="min-h-10"><Icon name="sliders" />列</Button></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuLabel>显示列</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuCheckboxItem v-model:checked="showCustomer">客户</DropdownMenuCheckboxItem><DropdownMenuCheckboxItem v-model:checked="showDate">日期</DropdownMenuCheckboxItem><DropdownMenuCheckboxItem v-model:checked="showChannel">渠道</DropdownMenuCheckboxItem></DropdownMenuContent></DropdownMenu>
        </div>
