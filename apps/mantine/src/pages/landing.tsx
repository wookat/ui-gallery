    ...
    <Box>
      <Box component="header" pos="sticky" top={0} style={{ zIndex: 10, borderBottom: "1px solid var(--mantine-color-default-border)", background: "var(--mantine-color-body)" }}>
        <Container size="lg" h={60}>
          <Group h="100%" justify="space-between">
            ...
            <Group gap="lg" visibleFrom="md">{navLinks.map((l) => <Anchor key={l} size="sm" c="dimmed" href="#">{l}</Anchor>)}</Group>
            <Group gap="xs">
              <ActionIcon variant="default" onClick={toggle} aria-label="切换主题"><Icon name={computed === "dark" ? "sun" : "moon"} size={16} /></ActionIcon>
              ...
              <Burger hiddenFrom="md" size="sm" opened={menuOpen} onClick={() => setMenuOpen(true)} aria-label="菜单" />
            </Group>
          </Group>
        </Container>
      </Box>
      ...
      <Container size="lg" py={{ base: 48, md: 96 }}>
        <Stack align="center" gap="lg" ta="center">
          ...
          <Text c="dimmed" size="lg" maw={600}>{landing.hero.subtitle}</Text>
          ...
          <Group gap="sm"><Avatar.Group>{landing.testimonials.slice(0, 4).map((t) => <Avatar key={t.name} radius="xl" color="initials" name={t.name} />)}</Avatar.Group><Text size="sm" c="dimmed">{landing.hero.social}</Text></Group>
          <AspectRatio ratio={16 / 9} w="100%" maw={960} mt="xl">
            <Center style={{ border: "1px solid var(--mantine-color-default-border)", borderRadius: 12, background: "var(--mantine-color-default)" }}><Stack align="center" gap="xs" c="dimmed"><Icon name="grid" size={40} /><Text size="sm">产品截图占位</Text></Stack></Center>
          </AspectRatio>
        </Stack>
      </Container>

      <Container size="lg" py="xl">
        <Text ta="center" size="sm" c="dimmed" mb="lg">受到各行业团队信任</Text>
        <SimpleGrid cols={{ base: 3, md: 6 }}>{Array.from({ length: 6 }).map((_, i) => <Center key={i} h={40} style={{ borderRadius: 8, background: "var(--mantine-color-default)" }}><Text size="xs" c="dimmed" fw={600}>LOGO {i + 1}</Text></Center>)}</SimpleGrid>
      </Container>

      <Container size="lg" py={{ base: 48, md: 80 }}>
        <Stack align="center" ta="center" mb="xl"><Title order={2}>一站式团队控制台</Title><Text c="dimmed" maw={560}>所有你需要的能力，开箱即用。</Text></Stack>
          ...
          {splits.map((f, i) => (
            <Grid key={f.title} align="center" gap="xl">
              <Grid.Col span={{ base: 12, md: 6 }} order={{ base: 1, md: i % 2 ? 2 : 1 }}>
                ...
                <Text c="dimmed" mt="sm">{f.desc}</Text>
              ...
              <Grid.Col span={{ base: 12, md: 6 }} order={{ base: 2, md: i % 2 ? 1 : 2 }}>
                <AspectRatio ratio={4 / 3}><Center style={{ border: "1px solid var(--mantine-color-default-border)", borderRadius: 12, background: "var(--mantine-color-default)" }}><Icon name={f.icon} size={48} /></Center></AspectRatio>
              </Grid.Col>
            </Grid>
          ))}
        </Stack>
        ...
        <SimpleGrid cols={{ base: 1, md: 3 }} spacing="lg">
          {plans.map((p) => (
            <Card key={p.name} withBorder radius="md" padding="xl" style={p.recommended ? { borderColor: "var(--mantine-primary-color-filled)", borderWidth: 2 } : undefined}>
              ...
              <Text fz={36} fw={700} mt="sm">{p.price === null ? "定制" : p.price === 0 ? "免费" : money(yearly ? Math.round(p.price * 12 * 0.8) : p.price)}{p.price ? <Text span size="sm" c="dimmed" fw={400}> /{yearly ? "年" : "月"}</Text> : null}</Text>
      ...
      <Container size="lg" py="xl">
        ...
        <Carousel slideSize={{ base: "100%", sm: "50%", md: "33.333%" }} slideGap="md" emblaOptions={{ align: "start", loop: true }} withIndicators>
          {landing.testimonials.map((t) => (
            <Carousel.Slide key={t.name}>
              <Card withBorder radius="md" padding="lg" h="100%">
                ...
                <Group mt="lg" gap="sm"><Avatar radius="xl" color="initials" name={t.name} /><div><Text size="sm" fw={500}>{t.name}</Text><Text size="xs" c="dimmed">{t.company}</Text></div></Group>
              </Card>
            </Carousel.Slide>
          ))}
        </Carousel>
      ...
      <Container size="sm" py={{ base: 48, md: 80 }}>
        ...
        <Accordion variant="separated">{landing.faq.map((f) => <Accordion.Item key={f.q} value={f.q}><Accordion.Control>{f.q}</Accordion.Control><Accordion.Panel><Text size="sm" c="dimmed">{f.a}</Text></Accordion.Panel></Accordion.Item>)}</Accordion>
      </Container>
      ...
      <Box component="footer" style={{ borderTop: "1px solid var(--mantine-color-default-border)" }} py="xl">
        <Container size="lg">
          <Grid gap="xl">
            <Grid.Col span={{ base: 12, md: 4 }}><Group gap="xs"><ThemeIcon radius="md"><Text fw={700} size="sm">A</Text></ThemeIcon><Text fw={600}>Acme Console</Text></Group><Text size="sm" c="dimmed" mt="sm">{landing.hero.subtitle}</Text></Grid.Col>
            {footerCols.map((c) => <Grid.Col key={c.title} span={{ base: 6, sm: 3, md: 2 }}><Text fw={600} size="sm" mb="sm">{c.title}</Text><Stack gap={6}>{c.links.map((l) => <Anchor key={l} size="sm" c="dimmed" href="#">{l}</Anchor>)}</Stack></Grid.Col>)}
          </Grid>
          <Divider my="lg" />
          <Group justify="space-between" wrap="wrap">
            <Text size="xs" c="dimmed">© 2026 Acme Console. 保留所有权利。</Text>
            <Group gap="sm"><ActionIcon variant="subtle" color="gray" aria-label="GitHub"><Icon name="github" size={16} /></ActionIcon><ActionIcon variant="subtle" color="gray" aria-label="Twitter"><Icon name="globe" size={16} /></ActionIcon><ActionIcon variant="subtle" color="gray" aria-label="Mail"><Icon name="send" size={16} /></ActionIcon><Select size="xs" w={120} data={["简体中文", "English"]} defaultValue="简体中文" allowDeselect={false} leftSection={<Icon name="globe" size={14} />} /></Group>
          </Group>
        </Container>
      </Box>
    </Box>
