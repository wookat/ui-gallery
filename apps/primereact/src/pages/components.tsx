...
function Demo({ name }: { name: string }) {
  ...
  if (name === "NumberInput") return <div className="flex flex-column gap-3"><InputNumber showButtons buttonLayout="horizontal" className="w-full md:w-14rem" inputClassName="w-full" value={Number(value ?? 2)} onValueChange={(event) => setValue(event.value)} /><InputNumber mode="currency" currency="CNY" locale="zh-CN" value={128430} /><InputNumber disabled value={10} /></div>
  ...
  if (name === "Table") return <DataTable value={orders.slice(0, 5)} paginator rows={5} responsiveLayout="stack" breakpoint="767px" className="w-full" stripedRows><Column field="id" header="订单号" sortable /><Column field="customer" header="客户" /><Column field="status" header="状态" sortable /><Column field="amount" header="金额" sortable body={(row) => `¥${row.amount.toLocaleString()}`} /></DataTable>
