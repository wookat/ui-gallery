...
<template>
  <div class="page">
    ...
    <el-card><div class="toolbar">
               <el-input v-model="query" clearable placeholder="搜索订单、客户或产品" class="query"><template #prefix><Icon name="search" /></template></el-input><el-select v-model="status" clearable placeholder="状态" style="width: 130px"><el-option label="已支付" value="paid" /><el-option label="待处理" value="pending" /><el-option
                 label="已退款"
                 value="refunded"
               /><el-option label="失败" value="failed" /></el-select><el-date-picker type="daterange" start-placeholder="开始日期" end-placeholder="结束日期" style="width: 240px" /><el-select
                 v-model="channels"
                 multiple
                 collapse-tags
                 placeholder="渠道"
                 style="width: 150px"
               ><el-option v-for="channel in ['web', 'ios', 'android', 'api']" :key="channel" :label="channel" :value="channel" /></el-select><el-popover placement="bottom-end" title="显示列" trigger="click"><template #reference><el-button><Icon name="sliders" />列</el-button></template><el-checkbox-group model-value="['id','customer','product','amount','status']"><el-checkbox value="id">订单号</el-checkbox><el-checkbox value="customer">客户</el-checkbox><el-checkbox value="product">产品</el-checkbox><el-checkbox value="amount">金额</el-checkbox></el-checkbox-group></el-popover>
             </div>
      ...
      <div v-else-if="mobile" class="order-cards">
        <el-card v-for="row in visible" :key="row.id" shadow="never" class="order-card" @click="selectOrder(row)"><div class="order-card-head">
                                                                                                                    <strong>{{ row.id }}</strong><span class="muted">{{ row.customer }}</span>
                                                                                                                  </div>
          <div>{{ row.product }}</div>
          <div class="order-card-meta muted">{{ row.channel }} · {{ row.date }}</div>
          <div class="order-card-foot">
            <el-tag :type="tagType(row.status)">{{ row.status }}</el-tag><strong>¥{{ row.amount.toLocaleString() }}</strong><el-button text type="primary" @click.stop="selectOrder(row)">查看</el-button>
          </div></el-card>
      </div>
      <div v-else>
