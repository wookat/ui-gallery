  ...
  <div class="chat-page">
    ...
    <div class="chat-layout">
      ...
      <section class="chat-main">
        ...
        <div v-else class="empty-state chat-empty"><AppIcon name="sparkles" :size="32" /><h2>你好，我是 Acme 助手</h2><p>选择一个建议，开始探索你的工作区数据。</p><div class="suggestion-grid"><Button v-for="suggestion in chat.suggestions" :key="suggestion" :label="suggestion" severity="secondary" outlined @click="draft = suggestion" /></div></div>
        <div class="composer"><div class="suggestions"><Chip v-for="suggestion in chat.suggestions" :key="suggestion" :label="suggestion" class="suggestion-chip" @click="draft = suggestion" /></div><div class="composer-row"><Button text rounded severity="secondary" aria-label="添加附件"><template #icon><AppIcon name="paperclip" :size="18" /></template></Button><Textarea v-model="draft" auto-resize rows="1" maxlength="2000" placeholder="输入消息，按 Enter 发送" fluid @keydown.enter.exact.prevent="send" /><Button rounded aria-label="发送" :disabled="!draft.trim()" @click="send"><template #icon><AppIcon name="send" :size="18" /></template></Button></div><div class="flex justify-between text-xs muted"><span>{{ draft.length }}/2000</span><span><span class="kbd">Enter</span> 发送 · <span class="kbd">Shift+Enter</span> 换行</span></div></div>
      </section>
    </div>
  </div>
</template>
...
.composer { padding: 12px 20px 16px; border-top: 1px solid var(--p-content-border-color); background: var(--p-content-background); } .suggestions { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 8px; } .suggestion-chip { cursor: pointer; max-width: 100%; } .composer-row { display: flex; align-items: flex-end; gap: 8px; } .composer-row > .p-button { flex: none; width: 2.5rem; height: 2.5rem; } .composer-row :deep(textarea) { max-height: 140px; } .chat-empty { flex: 1; min-height: 360px; }
