...
export function ComponentsPage() {
  return <div className="flex flex-column gap-5"><PageHeader title="组件全集" description="PrimeReact 官方组件、contract 覆盖与组合示例。" action={<Button outlined label="组件索引" icon={<Icon name="list" />} />} /><div id="component-index" className="flex flex-wrap gap-2">{(contract.components as string[]).map((name) => <a className="p-button p-button-outlined p-button-sm no-underline" href={`#component-${name}`} key={name}>{name}</a>)}</div><div className="grid">{(contract.components as string[]).map((name) => <div className="col-12 md:col-6 xl:col-4" id={`component-${name}`} key={name}><Card className="demo-card" title={<span className="flex justify-content-between align-items-center">{name}<Tag value={coverage[name]} severity={coverage[name] === "missing" ? "danger" : coverage[name] === "composed" ? "warning" : "success"} /></span>} subTitle="default · disabled · loading · error"><Demo name={name} /></Card></div>)}</div><div><h2>Extras</h2><div className="grid">{extras.map((name) => <div className="col-12 md:col-4" key={name}><Card title={name}><p className="muted m-0">PrimeReact 官方导出组件</p></Card></div>)}</div><Message severity="info" text="Editor 需要 Quill，按约定跳过。" /></div></div>
}
