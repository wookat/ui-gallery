...
export function Combobox(props: { label?: string; options: SelectOption[] | string[]; placeholder?: string; value?: string; onChange?: (value: string | null) => void }) {
  ...
  return (
    <KobalteCombobox options={options} optionValue="value" optionLabel="label" optionTextValue="label" value={props.value ? options.find((item) => item.value === props.value) : null} onChange={props.onChange as never} itemComponent={(item) => <KobalteCombobox.Item item={item.item} class="rounded px-2 py-1.5 text-sm data-[highlighted]:bg-zinc-100 dark:data-[highlighted]:bg-zinc-800">{item.item.rawValue.label}</KobalteCombobox.Item>}>
