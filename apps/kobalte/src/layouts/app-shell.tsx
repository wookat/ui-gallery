...
export function AppShell(props: ParentProps) {
  ...
  const path = () => { const p = location.pathname.replace(/^\/apps\/kobalte/, "") || "/"; return p.length > 1 ? p.replace(/\/$/, "") : p }
  const current = () => nav.find((item) => item.path === path())?.label ?? "仪表盘"
  const toggleTheme = () => {
      ...
      <For each={nav}>
        {(item) => (
          <Tooltip.Root openDelay={300}>
            ...
            <A href={item.path} onClick={() => setMobileOpen(false)} class={`flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors ${path() === item.path ? "bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900" : "text-zinc-600 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800"}`}>
  ...
  return (
    <div class="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <aside class={`fixed inset-y-0 left-0 z-30 hidden border-r border-zinc-200 bg-white transition-[width] lg:flex lg:flex-col dark:border-zinc-800 dark:bg-zinc-900 ${collapsed() ? "w-16" : "w-64"}`}>
        ...
        <div class="flex-1 space-y-6 overflow-y-auto p-3"><Show when={!collapsed()}><p class="px-3 text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">工作区</p></Show>{links()}</div>
        ...
        <header class="sticky top-0 z-20 flex h-16 min-w-0 items-center gap-3 border-b border-zinc-200 bg-white/90 px-4 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/90 sm:px-6">
          ...
          <div class="ml-auto flex min-w-0 items-center gap-2">
            ...
            <Popover.Root>
              ...
              <Popover.Portal><Popover.Content class="z-50 w-80 rounded-lg border border-zinc-200 bg-white p-4 shadow-lg dark:border-zinc-700 dark:bg-zinc-900"><p class="mb-3 font-medium">通知</p><div class="grid gap-3"><For each={notifications}>{(item) => <div class="border-b border-zinc-100 pb-3 text-sm last:border-0 dark:border-zinc-800"><p>{item.title}</p><p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{item.time}</p></div>}</For></div></Popover.Content></Popover.Portal>
            </Popover.Root>
