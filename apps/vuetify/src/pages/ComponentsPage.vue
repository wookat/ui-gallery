...
<template>
  <div>
    ...
    <v-chip-group class="mb-6" selected-class="text-primary" show-arrows><v-chip v-for="item in anchors" :key="item" :href="`#sec-${item}`" size="small" variant="outlined">{{ item }}</v-chip></v-chip-group>
        ...
        <v-col v-for="name in section.items" :id="`sec-${name}`" :key="name" cols="12" md="6" xl="4">
          <v-card :title="name" :subtitle="coverage[name as keyof typeof coverage]">
            <v-card-text>
              ...
              <template v-else-if="name === 'Image'"><v-img :src="imageSrc" height="120" cover eager @click="dialog = true" /></template>
              ...
              <template v-else-if="['Navbar', 'Sidebar'].includes(name)"><v-toolbar v-if="name === 'Navbar'" flat border density="comfortable"><v-app-bar-nav-icon /><v-toolbar-title>导航栏</v-toolbar-title><v-spacer /><v-btn icon variant="text"><Icon name="settings" /></v-btn></v-toolbar><v-sheet v-else class="d-flex" height="120"><v-sheet width="140" color="surface-variant" class="pa-3">侧边栏</v-sheet><v-sheet class="flex-grow-1" /></v-sheet></template>
    ...
    <v-dialog v-model="dialog" max-width="560"><v-card title="预览"><v-img :src="imageSrc" eager /><v-card-actions><v-spacer /><v-btn @click="dialog = false">关闭</v-btn></v-card-actions></v-card></v-dialog>
