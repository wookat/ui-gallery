...
export function LandingPage() {
  ...
  return <>
    ...
    <Container>
      <Row className="align-items-center py-5 g-4"><Col lg={6}><Badge bg="primary-subtle" text="primary-emphasis" className="mb-3">新一代团队工作台</Badge><h1 className="display-4 fw-bold">{landing.hero.title}</h1><p className="lead text-body-secondary">{landing.hero.subtitle}</p><div className="d-flex flex-wrap gap-2"><Link className="btn btn-primary btn-lg" to="/login">{landing.hero.primary}</Link><Button size="lg" variant="outline-secondary">{landing.hero.secondary}</Button></div><div className="d-flex align-items-center gap-3 mt-4"><AvatarGroup names={["林晓", "Alex", "Maria"]} /><span className="text-body-secondary">{landing.hero.social}</span></div></Col><Col lg={6}><div className="hero-placeholder bg-body-secondary rounded-4 d-flex align-items-center justify-content-center" style={{ minHeight: 300 }}><Icon name="layout-dashboard" size={48} /></div></Col></Row>
      <div className="d-flex flex-wrap justify-content-around gap-3 border-top border-bottom py-4 text-body-secondary fw-semibold">{["Northwind", "Contoso", "Globex", "Fabrikam", "Initech", "Umbrella"].map((name) => <span key={name}>{name}</span>)}</div>
      <Section title="为团队而生" className="py-5"><Row id="features" className="g-3">{landing.features.map((feature) => <Col md={6} lg={4} key={feature.title}><Card className="h-100 border-0 bg-body-tertiary"><Card.Body><Icon name={feature.icon} className="text-primary mb-3" size={25} /><h2 className="h5">{feature.title}</h2><p className="text-body-secondary mb-0">{feature.desc}</p></Card.Body></Card></Col>)}</Row></Section>
    ...
    <footer className="footer-links border-top py-5"><Container><Row className="g-4"><Col md={4}><Brand /><p className="text-body-secondary mt-3">© 2026 Acme Console</p><div className="d-flex gap-2"><Button variant="outline-secondary" aria-label="github"><Icon name="github" /></Button><Button variant="outline-secondary" aria-label="globe"><Icon name="globe" /></Button></div></Col>{["产品", "资源", "公司", "支持"].map((title) => <Col xs={6} md key={title}><h2 className="h6">{title}</h2><a className="d-block text-body-secondary" href="#features">特性</a><a className="d-block text-body-secondary" href="#pricing">定价</a><a className="d-block text-body-secondary" href="#faq">帮助中心</a></Col>)}<Col xs={6} md={2}><Form.Select aria-label="语言"><option>简体中文</option><option>English</option></Form.Select></Col></Row></Container></footer>
  </>
}
