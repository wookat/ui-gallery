 ...
 * 水平且带 description / onStepChange 时为 form 稿的可点步骤（done primary-soft + 勾、current primary 实底、error danger-soft + 警示，space.10 短连接线）。
 */
function Stepper({ steps, current, statusLabels, compactLabel, errorAt, onStepChange, reached, disabled, orientation = "horizontal", className, ...props }: StepperProps) {
  const vertical = orientation === "vertical"
  const statusOf = (i: number): StepStatus => (i === errorAt ? "error" : i < current ? "done" : i === current ? "current" : "todo")
  const rich = !vertical && (steps.some((s) => s.description) || !!onStepChange)
  ...
  return (
    <div data-slot="stepper" data-orientation={orientation} className={cn("flex w-full flex-col gap-2", className)}>
      <ol className={cn("flex w-full", vertical ? "flex-col items-stretch gap-2" : rich ? "items-start gap-3 px-3" : "items-center")} {...props}>
        {steps.map((step, i) => {
          ...
          const dot = (
            ...
            >
              {status === "done" ? <CheckIcon strokeWidth={3} /> : status === "error" ? vertical ? <XIcon strokeWidth={3} /> : <CircleAlertIcon /> : i + 1}
            </span>
          )
          if (!rich) {
            return (
          ...
          return (
            <React.Fragment key={step.key}>
              ...
              {!last ? <li aria-hidden className={cn("mt-[calc(var(--space-2)+var(--size-step)/2)] h-hairline w-10 shrink-0", i < current ? "bg-primary" : "bg-border")} /> : null}
            </React.Fragment>
          )
        })}
      </ol>
