...
const pieOption = computed(() => ({ tooltip: { trigger: "item" }, series: [{ type: "pie", radius: ["35%", "62%"], center: ["50%", "50%"], data: series.byChannel.map((item) => ({ name: item.name, value: item.value })) }] }))
...
.recent-cards { display: grid; gap: 12px; }.recent-card { display: flex; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid var(--el-border-color-lighter); }.recent-card > div { display: grid; gap: 6px; }.recent-card > div:last-child { justify-items: end; }
</style>
