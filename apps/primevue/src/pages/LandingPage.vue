...
const links = ["产品", "解决方案", "资源", "定价", "关于我们"]
const featureSections = landing.features.slice(0, 3)
</script>

<template>
  <div class="landing">
    <header class="landing-nav container"><a href="#" class="landing-brand"><span class="landing-logo">A</span><strong>Acme Console</strong></a><nav class="landing-links desktop-only"><a v-for="link in links" :key="link" href="#features">{{ link }}</a></nav><div class="desktop-only"><Button label="免费开始" size="small" @click="$router.push('/login')" /></div><Button class="mobile-only" icon="pi pi-bars" text rounded aria-label="打开菜单" @click="mobileOpen = true" /></header>
    ...
    <main>
      <section class="hero container"><div class="hero-copy"><Tag value="Acme Console" severity="secondary" /><h1>{{ landing.hero.title }}</h1><p>{{ landing.hero.subtitle }}</p><div class="flex wrap gap-2"><Button :label="landing.hero.primary" @click="$router.push('/login')" /><Button :label="landing.hero.secondary" severity="secondary" outlined /></div><div class="flex items-center gap-3 mt-6"><AvatarGroup><Avatar v-for="name in ['林', '王', 'A', 'M']" :key="name" :label="name" shape="circle" /></AvatarGroup><span class="text-sm muted">{{ landing.hero.social }}</span></div></div><div class="product-placeholder"><div class="placeholder-bar" /><div class="placeholder-grid"><div v-for="n in 6" :key="n" class="placeholder-card" /></div></div></section>
      <section class="logo-cloud container"><span v-for="n in 6" :key="n">Logo</span></section>
      <section id="features" class="landing-section container"><div class="section-intro"><Tag value="功能" /><h2>一个控制台，连接整个团队</h2></div><div class="grid grid-3"><Card v-for="feature in landing.features" :key="feature.title"><template #content><AppIcon :name="feature.icon as any" :size="24" /><h3 class="mt-4 font-semibold">{{ feature.title }}</h3><p class="text-sm muted mt-2">{{ feature.desc }}</p></template></Card></div></section>
      <section class="landing-section container split-sections"><div v-for="(feature, index) in featureSections" :key="feature.title" class="split-row" :class="{ reverse: index % 2 === 1 }"><div class="split-placeholder"><AppIcon :name="feature.icon as any" :size="40" /></div><div><Tag :value="`0${index + 1}`" severity="secondary" /><h2 class="mt-3">{{ feature.title }}</h2><p class="muted mt-2">{{ feature.desc }}</p><a href="#pricing" class="inline-flex items-center gap-2 mt-4">了解更多 <i class="pi pi-arrow-right" /></a></div></div></section>
    ...
    <footer class="landing-footer container"><div><a href="#" class="landing-brand"><span class="landing-logo">A</span><strong>Acme Console</strong></a><p class="text-sm muted mt-3">让团队工作更简单。</p></div><div v-for="column in [['产品', '概览', '功能', '定价'], ['资源', '文档', '帮助中心', '更新日志'], ['公司', '关于我们', '联系我们', '招聘']]" :key="column[0]" class="footer-col"><strong>{{ column[0] }}</strong><a v-for="item in column.slice(1)" :key="item" href="#">{{ item }}</a></div><div><strong>语言</strong><Select :options="['简体中文', 'English']" model-value="简体中文" class="mt-2" /></div></footer>
  </div>
</template>
...
.landing-links a, .landing-mobile-links a, .footer-col a { color: var(--p-text-muted-color); text-decoration: none; transition: color .15s ease; }
...
.placeholder-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; } .placeholder-card { height: 100px; border-radius: 10px; background: var(--p-content-background); border: 1px solid var(--p-content-border-color); }
.logo-cloud { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 12px; padding-bottom: 80px; } .logo-cloud span { padding: 10px 28px; border-radius: 8px; background: var(--p-surface-100); color: var(--p-text-muted-color); }
